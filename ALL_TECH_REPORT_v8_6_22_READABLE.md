...
**Масштаб процесса:** 58 взаимодействий, из них 51 основное взаимодействие и 7 сквозных контролей. Участников: 61.
...
|---|---|---|---|
| Критический | Финансовую сущность изменяют несколько систем одновременно. | Audit journal, Сервис процесса, Сервис телеметрии | Назначьте единственного писателя для счёта или шарда и ведите неизменяемый журнал проводок; остальные системы должны отправлять команды, а не менять финансовое состояние напрямую. |
...
**Где:** связь идёт от «Сервис процесса» к «Внешний партнёр». Исполнитель: «Сервис процесса».
...
| Классифицировать и маскировать чувствительные поля | Сервис процесса → Слой защиты и маскирования; при необходимости применяется ко всему процессу | контроль работает сквозно и не меняет порядок бизнес-шагов |
| Записать неизменяемый audit journal | Сервис процесса → Audit journal; при необходимости применяется ко всему процессу | контроль работает сквозно и не меняет порядок бизнес-шагов |
...
|---|---|---|
| Финансовую сущность изменяют несколько систем одновременно. | Audit journal, Сервис процесса, Сервис телеметрии | Назначьте единственного писателя для счёта или шарда и ведите неизменяемый журнал проводок; остальные системы должны отправлять команды, а не менять финансовое состояние напрямую. |
...
Матрица деталей: применимо инвариантов из каталога v7.1 — 125 из 125; блокируют выпуск — 17, требуют внимания — 29, нужно уточнить — 39, уже выглядит закрытым — 55.
