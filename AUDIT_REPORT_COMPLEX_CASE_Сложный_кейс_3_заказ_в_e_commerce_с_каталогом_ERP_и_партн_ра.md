...
|---|---|---|---|
| API и онлайн-взаимодействие | шаг 1: Web/Mobile → Каталог — Клиент получает каталог через гибкий API (GraphQL); шаг 5: Каталог → GraphQL BFF — Каталог читает редко меняющиеся данные из простого временного кэша (GraphQL); шаг 7: GraphQL BFF → БД заказов — Заказ сохраняется в основную БД (GraphQL); ещё 1 | входящий или внутренний запрос получает быстрый ответ либо идентификатор отслеживания для дальнейшего отслеживания | Нужны авторизация полей, лимит глубины/сложности запроса, пагинация, защита от тяжёлых запросов и понятный контракт схемы; Нужны авторизация полей, лимит глубины/сложности запроса, пагинация, защита от тяжёлых запросов и понятный контракт схемы; ещё 2 |
...
| Прочее | шаг 6: Каталог → Каталог — ERP отдаёт корпоративные сущности через OData (сначала исправить схему); шаг 9: GraphQL BFF → GraphQL BFF — Идемпотентный быстрый доступ к корзине идёт по ключу (сначала исправить схему); шаг 10: GraphQL BFF → GraphQL BFF — Гибкий документ состава заказа хранится как документ (сначала исправить схему) | дополнительные взаимодействия описаны отдельно и требуют ручного уточнения владельцев и результата | Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека; Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека; ещё 1 |
...
CREATE INDEX idx_order_идентификатор сквозной связи ON order (идентификатор сквозной связи);
...
| Контракты | Не указано | — | Event-as-command смешивает ответственность и приводит к непредсказуемым side effects. | — |
...
| Безопасность | Проверено | — | Без политики срока хранения данные остаются навсегда во вспомогательных местах. | — |
...
| Тестирование | Проверено | — | Дубли являются нормой для at-least-once. | — |
