...
|---|---|---|
| Пользователь подтверждает доступ через единую авторизацию | Мобильное приложение → API Gateway; при необходимости применяется ко всему процессу | контроль работает сквозно и не меняет порядок бизнес-шагов |
| События выплаты читают уведомления, аудит и аналитика | Сервис страховой заявки → Kafka; при необходимости применяется ко всему процессу | контроль работает сквозно и не меняет порядок бизнес-шагов |
| Секреты партнёров и ключи подписи хранятся защищённо | Сервис страховой заявки → REST антифрод; при необходимости применяется ко всему процессу | контроль работает сквозно и не меняет порядок бизнес-шагов |
| Наблюдаемость показывает зависшие выплаты и ошибки партнёров | Сервис страховой заявки → Observability; при необходимости применяется ко всему процессу | контроль работает сквозно и не меняет порядок бизнес-шагов |
