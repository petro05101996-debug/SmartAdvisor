...
**Где:** Основные проблемные места: аналитическое хранилище или аналитика находятся в операционном основной поток — в зоне: Шаг 18 «Аналитическое хранилище хранит согласованные витрины» → аналитическое хранилище; Система одновременно пишет в БД и публикует событие без таблица исходящих сообщений — в зоне: Сервис анкеты: «Сервис публикует событие о создании анкеты нескольким потребителям», «OCR получает фоновую задачу на обработку документа»; Входящий входящий веб-вызов должен проходить проверку подлинности — в зоне: Шаг 15 «Партнёр позже присылает статус проверки».
...
**Что:** в шаге 1 «Клиент открывает статические ресурсы анкеты» предлагается использовать CDN.
**Где:** это применяется на связи CDN → Клиентское приложение; шаг выполняется после: начало процесса или внешний запуск; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 2 «Клиент входит через единый контур авторизации» предлагается использовать OAuth2/OIDC.
**Где:** это применяется на связи Клиентское приложение → Сервис авторизации → API Gateway; шаг выполняется после: шаг 1 «Клиент открывает статические ресурсы анкеты»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 3 «Внешний вход проверяет токен и лимиты» предлагается использовать API Gateway.
**Где:** это применяется на связи Клиентское приложение → API Gateway → Сервис анкеты; шаг выполняется после: шаг 2 «Клиент входит через единый контур авторизации»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 4 «Внутренний вызов проходит через управляемый сервисный контур» предлагается использовать Service Mesh.
**Где:** это применяется на связи API Gateway → Service Mesh → Сервис анкеты; шаг выполняется после: шаг 3 «Внешний вход проверяет токен и лимиты»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 5 «Сервис анкеты быстро получает профиль клиента» предлагается использовать gRPC.
**Где:** это применяется на связи Сервис анкеты → Профильный сервис; шаг выполняется после: шаг 4 «Внутренний вызов проходит через управляемый сервисный контур»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 6 «Профиль часто читается из кэша» предлагается использовать Redis как кэш.
**Где:** это применяется на связи Профильный сервис → Redis кэш → Сервис анкеты; шаг выполняется после: шаг 5 «Сервис анкеты быстро получает профиль клиента»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 7 «Одну анкету нельзя обрабатывать параллельно» предлагается использовать Redis как распределённая блокировка.
**Где:** это применяется на связи Сервис анкеты → Redis блокировка → Сервис анкеты; шаг выполняется после: шаг 6 «Профиль часто читается из кэша»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 8 «Сервис сверяет клиента со старой АБС по старому контракту» предлагается использовать SOAP.
**Где:** это применяется на связи Сервис анкеты → Legacy АБС; шаг выполняется после: шаг 7 «Одну анкету нельзя обрабатывать параллельно»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 9 «Сервис сохраняет состояние анкеты» предлагается использовать Основная база данных.
**Где:** это применяется на связи Сервис анкеты → БД процесса; шаг выполняется после: шаг 8 «Сервис сверяет клиента со старой АБС по старому контракту»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 10 «Чтение списка анкет разгружается на реплику» предлагается использовать Реплика базы данных для чтения.
**Где:** это применяется на связи БД процесса → Реплика чтения → Сервис анкеты; шаг выполняется после: шаг 9 «Сервис сохраняет состояние анкеты»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 11 «Поиск клиентов идёт через поисковый индекс» предлагается использовать Поисковый индекс.
**Где:** это применяется на связи БД процесса → Поисковый индекс → Сервис анкеты; шаг выполняется после: шаг 9 «Сервис сохраняет состояние анкеты»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 12 «Скан паспорта хранится как объект, в событиях передаётся ссылка» предлагается использовать Объектное хранилище.
**Где:** это применяется на связи Сервис анкеты → Объектное хранилище; шаг выполняется после: шаг 9 «Сервис сохраняет состояние анкеты»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 13 «Сервис публикует событие о создании анкеты нескольким потребителям» предлагается использовать Kafka.
**Где:** это применяется на связи Сервис анкеты → Kafka; шаг выполняется после: шаг 9 «Сервис сохраняет состояние анкеты»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 14 «OCR получает фоновую задачу на обработку документа» предлагается использовать RabbitMQ.
**Где:** это применяется на связи Сервис анкеты → RabbitMQ; шаг выполняется после: шаг 12 «Скан паспорта хранится как объект, в событиях передаётся ссылка»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 15 «Партнёр позже присылает статус проверки» предлагается использовать Входящий веб-вызов.
**Где:** это применяется на связи Входящий веб-вызов партнёра → API Gateway → Сервис анкеты; шаг выполняется после: шаг 13 «Сервис публикует событие о создании анкеты нескольким потребителям»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 16 «Изменения состояния передаются в аналитику» предлагается использовать Передача изменений из базы данных.
**Где:** это применяется на связи БД процесса → аналитическое хранилище; шаг выполняется после: шаг 9 «Сервис сохраняет состояние анкеты»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 17 «Регламентная сверка аналитическое хранилище запускается пачкой по расписанию» предлагается использовать Пакетная обработка.
**Где:** это применяется на связи участник: аналитическое хранилище; шаг выполняется после: шаг 16 «Изменения состояния передаются в аналитику»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 18 «Аналитическое хранилище хранит согласованные витрины» предлагается использовать Аналитическое хранилище.
**Где:** это применяется на связи участник: аналитическое хранилище; шаг выполняется после: шаг 17 «Регламентная сверка аналитическое хранилище запускается пачкой по расписанию»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 19 «Секреты внешних интеграций берутся из защищённого хранилища» предлагается использовать Vault/KMS для секретов и ключей.
**Где:** это применяется на связи Сервис анкеты → Vault/KMS → Legacy АБС; шаг выполняется после: шаг 8 «Сервис сверяет клиента со старой АБС по старому контракту»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 20 «Сквозные метрики и трассировки показывают, где зависла анкета» предлагается использовать Наблюдаемость.
**Где:** это применяется на связи Сервис анкеты → Observability; шаг выполняется после: шаг 15 «Партнёр позже присылает статус проверки»; характер взаимодействия — с ожиданием ответа сразу.
...
2. **Система одновременно пишет в БД и публикует событие без таблица исходящих сообщений.** — Сервис анкеты: «Сервис публикует событие о создании анкеты нескольким потребителям», «OCR получает фоновую задачу на обработку документа».
...
3. **Входящий входящий веб-вызов должен проходить проверку подлинности.** — Шаг 15 «Партнёр позже присылает статус проверки».
...
17. Регресс на «Система одновременно пишет в БД и публикует событие без таблица исходящих сообщений.»: Используйте Transactional таблица исходящих сообщений: событие записывается в той же транзакции, что и агрегат, а отдельный publisher вычитывает outbox и публикует событие с повторная попытка.
18. Регресс на «Входящий входящий веб-вызов должен проходить проверку подлинности.»: Проверяйте HMAC или подпись провайдера до любой бизнес-обработки; секрет храните в защищённом хранилище и предусмотрите его ротацию.
...
24. Регресс на «Дочерний вызов может ждать дольше, чем родительский шаг.»: Таймауты должны строго убывать вниз по цепочке: дочерний timeout должен быть меньше родительского с учётом сетевых накладных; общий бюджет времени распределяйте от требование ко времени ответа сверху вниз.
