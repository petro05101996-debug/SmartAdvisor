...
**Что:** в шаге 1 «Датчики отправляют телеметрию лёгким протоколом» предлагается использовать MQTT.
**Где:** это применяется на связи Устройства → MQTT broker; шаг выполняется после: начало процесса или внешний запуск; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 2 «Сырые события складываются в масштабный журнал событий» предлагается использовать Pulsar.
**Где:** это применяется на связи MQTT broker → Pulsar; шаг выполняется после: шаг 1 «Датчики отправляют телеметрию лёгким протоколом»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 3 «Короткий поток последних измерений идёт во внутренние обработчики» предлагается использовать Redis Streams.
**Где:** это применяется на связи Pulsar → Redis Streams → Сервис тревог; шаг выполняется после: шаг 2 «Сырые события складываются в масштабный журнал событий»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 4 «Лёгкая внутренняя рассылка уведомляет сервисы о тревоге» предлагается использовать NATS.
**Где:** это применяется на связи Сервис тревог → NATS → Сервис тревог; шаг выполняется после: шаг 3 «Короткий поток последних измерений идёт во внутренние обработчики»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 5 «Огромные записи по устройству хранятся по ключу устройства» предлагается использовать Cassandra/ScyllaDB.
**Где:** это применяется на связи Pulsar → Cassandra; шаг выполняется после: шаг 2 «Сырые события складываются в масштабный журнал событий»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 6 «Аналитические агрегаты пишутся в колоночную БД» предлагается использовать Колоночная аналитическая база данных.
**Где:** это применяется на связи Pulsar → ClickHouse; шаг выполняется после: шаг 2 «Сырые события складываются в масштабный журнал событий»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 7 «Сырые файлы телеметрии складываются в озеро данных» предлагается использовать Озеро данных.
**Где:** это применяется на связи Pulsar → Data Lake; шаг выполняется после: шаг 2 «Сырые события складываются в масштабный журнал событий»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 8 «Большая распределённая обработка пересчитывает историю» предлагается использовать Spark.
**Где:** это применяется на связи Data Lake → Spark → ClickHouse; шаг выполняется после: шаг 7 «Сырые файлы телеметрии складываются в озеро данных»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 9 «Зависимые загрузки управляются расписанием» предлагается использовать Airflow как оркестратор загрузок.
**Где:** это применяется на связи Spark → Airflow → dbt; шаг выполняется после: шаг 8 «Большая распределённая обработка пересчитывает историю»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 10 «Аналитические модели собираются управляемо» предлагается использовать dbt как слой аналитических моделей.
**Где:** это применяется на связи ClickHouse → dbt → ClickHouse; шаг выполняется после: шаг 9 «Зависимые загрузки управляются расписанием»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 11 «Оператор получает онлайн-тревоги в двустороннем канале» предлагается использовать WebSocket.
**Где:** это применяется на связи Сервис тревог → WebSocket канал → Устройства; шаг выполняется после: шаг 4 «Лёгкая внутренняя рассылка уведомляет сервисы о тревоге»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 12 «Панель мониторинга получает поток уведомлений» предлагается использовать Server-Sent Events.
**Где:** это применяется на связи Сервис тревог → SSE канал; шаг выполняется после: шаг 4 «Лёгкая внутренняя рассылка уведомляет сервисы о тревоге»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 13 «Наблюдаемость контролирует лаги и потерянные сообщения» предлагается использовать Наблюдаемость.
**Где:** это применяется на связи Pulsar → Observability; шаг выполняется после: шаг 2 «Сырые события складываются в масштабный журнал событий»; характер взаимодействия — с ожиданием ответа сразу.
