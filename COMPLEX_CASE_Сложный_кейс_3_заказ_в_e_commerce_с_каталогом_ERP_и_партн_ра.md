...
**Что:** в шаге 1 «Клиент получает каталог через гибкий API» предлагается использовать GraphQL.
**Где:** это применяется на связи Web/Mobile → GraphQL BFF → Каталог; шаг выполняется после: начало процесса или внешний запуск; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 2 «Статика и изображения товаров отдаются быстро» предлагается использовать CDN.
**Где:** это применяется на связи CDN → Web/Mobile; шаг выполняется после: шаг 1 «Клиент получает каталог через гибкий API»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 3 «Поиск товара идёт по поисковому индексу» предлагается использовать Поисковый индекс.
**Где:** это применяется на связи GraphQL BFF → Search index → Web/Mobile; шаг выполняется после: шаг 1 «Клиент получает каталог через гибкий API»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 4 «Семантический поиск похожих товаров использует векторы» предлагается использовать Векторное хранилище.
**Где:** это применяется на связи GraphQL BFF → Векторный поиск → Web/Mobile; шаг выполняется после: шаг 1 «Клиент получает каталог через гибкий API»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 5 «Каталог читает редко меняющиеся данные из простого временного кэша» предлагается использовать Memcached как простой временный кэш.
**Где:** это применяется на связи Каталог → Memcached → GraphQL BFF; шаг выполняется после: шаг 1 «Клиент получает каталог через гибкий API»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 6 «ERP отдаёт корпоративные сущности через OData» предлагается использовать OData.
**Где:** это применяется на связи Каталог → OData ERP → Каталог; шаг выполняется после: шаг 1 «Клиент получает каталог через гибкий API»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 7 «Заказ сохраняется в основную БД» предлагается использовать Основная база данных.
**Где:** это применяется на связи GraphQL BFF → БД заказов; шаг выполняется после: шаг 1 «Клиент получает каталог через гибкий API»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 8 «Данные заказов разделяются по ключу клиента» предлагается использовать Шардирование базы данных.
**Где:** это применяется на связи БД заказов → Шардированное хранилище; шаг выполняется после: шаг 7 «Заказ сохраняется в основную БД»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 9 «Идемпотентный быстрый доступ к корзине идёт по ключу» предлагается использовать Хранилище ключ-значение.
**Где:** это применяется на связи GraphQL BFF → DynamoDB → GraphQL BFF; шаг выполняется после: шаг 7 «Заказ сохраняется в основную БД»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 10 «Гибкий документ состава заказа хранится как документ» предлагается использовать Документное хранилище.
**Где:** это применяется на связи GraphQL BFF → MongoDB → GraphQL BFF; шаг выполняется после: шаг 7 «Заказ сохраняется в основную БД»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 11 «Короткая внутренняя задача пересчитывает рекомендации» предлагается использовать Redis как короткая очередь задач.
**Где:** это применяется на связи GraphQL BFF → Redis queue → Каталог; шаг выполняется после: шаг 7 «Заказ сохраняется в основную БД»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 12 «Если брокер ещё не выбран, задача уходит в нейтральную очередь» предлагается использовать Очередь сообщений, брокер пока не выбран.
**Где:** это применяется на связи GraphQL BFF → Generic queue → Каталог; шаг выполняется после: шаг 7 «Заказ сохраняется в основную БД»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 13 «Событие о заказе публикуется многим потребителям» предлагается использовать Kafka.
**Где:** это применяется на связи GraphQL BFF → Kafka; шаг выполняется после: шаг 7 «Заказ сохраняется в основную БД»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 14 «В корпоративном Microsoft-контуре заказ передаётся в очередь» предлагается использовать Azure Service Bus.
**Где:** это применяется на связи GraphQL BFF → Azure Service Bus → OData ERP; шаг выполняется после: шаг 13 «Событие о заказе публикуется многим потребителям»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 15 «Партнёр получает защищённый файл по SFTP» предлагается использовать SFTP.
**Где:** это применяется на связи Файловый шлюз → SFTP партнёра; шаг выполняется после: шаг 13 «Событие о заказе публикуется многим потребителям»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 16 «Одиночный файл накладной передаётся отдельным файлом» предлагается использовать Файловая передача.
**Где:** это применяется на связи Файловый шлюз → SFTP партнёра; шаг выполняется после: шаг 13 «Событие о заказе публикуется многим потребителям»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 17 «Большие вложения заказа хранятся отдельно» предлагается использовать Объектное хранилище.
**Где:** это применяется на связи GraphQL BFF → Object Storage; шаг выполняется после: шаг 7 «Заказ сохраняется в основную БД»; характер взаимодействия — без ожидания немедленного ответа.
...
| 5. Каталог читает редко меняющиеся данные из простого временного кэша. Маршрут: Каталог → Memcached → GraphQL BFF. | Memcached как простой временный кэш | Подходит для простого временного кэша без сложных структур, когда нужна высокая скорость чтения и допустима потеря кэша. | Redis богаче по структурам данных и сценариям блокировок/streams. БД остаётся источником истины, но не должна держать весь горячий read-трафик. | Нужны TTL, стратегия инвалидации, запасной сценарий к источнику, лимит размера значений и защита от лавина одновременных обращений к источнику данных. |
...
| 7. Заказ сохраняется в основную БД. Маршрут: GraphQL BFF → GraphQL BFF → БД заказов. | Основная база данных | Подходит для фиксации состояния процесса, статусов, ключей идемпотентности, истории и технического журнала шагов. | Redis не должен быть источником истины. Kafka/RabbitMQ передают сообщения, но не заменяют надёжную операционную запись. аналитическое хранилище не подходит для оперативной транзакции. | Нужны транзакции, уникальные индексы, версия записи или optimistic locking, сроки хранения и план очистки технических таблиц. |
...
| 9. Идемпотентный быстрый доступ к корзине идёт по ключу. Маршрут: GraphQL BFF → DynamoDB → GraphQL BFF. | Хранилище ключ-значение | Подходит для быстрого доступа по ключу в управляемом key-value/NoSQL-хранилище с предсказуемыми access patterns. | Реляционная БД лучше для сложных связей и транзакционных запросов. Cassandra чаще выбирается для self-hosted распределённой модели. Redis не источник истины для долговременных бизнес-данных. | Нужны ключ партиционирования, sort key, conditional write, TTL, лимиты throughput и защита от hot keys. |
| 10. Гибкий документ состава заказа хранится как документ. Маршрут: GraphQL BFF → MongoDB → GraphQL BFF. | Документное хранилище | Подходит для гибких документов и меняющейся структуры, когда сущность удобнее хранить как документ, а не как набор жёстких таблиц. | Реляционная БД лучше для строгих связей, транзакций и отчётности по нормализованной модели. Key-value проще, но хуже для сложного документа и индексов по полям. | Нужны схема документа, индексы, правила миграции структуры, лимит размера документа и стратегия консистентности. |
...
| 14. В корпоративном Microsoft-контуре заказ передаётся в очередь. Маршрут: GraphQL BFF → Azure Service Bus → OData ERP. | Azure Service Bus | Подходит для управляемой очереди/топика в Microsoft Azure, особенно если уже используется Azure AD, Logic Apps или корпоративный Microsoft-контур. | RabbitMQ/Kafka требуют отдельного сопровождения. AWS SNS/SQS или Google Pub/Sub выбираются в других облаках. Redis queue слабее для критичной enterprise-очереди. | Нужны namespace, queue/topic/subscription, dead-letter queue, lock duration, повторная попытка policy, RBAC и мониторинг. |
