...
**Что:** в шаге 1 «Старый фронт входит через единый внешний вход» предлагается использовать API Gateway.
**Где:** это применяется на связи Старый фронт → API Gateway → ESB; шаг выполняется после: начало процесса или внешний запуск; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 2 «Шина маршрутизирует запрос в несколько старых систем» предлагается использовать Интеграционная шина ESB.
**Где:** это применяется на связи API Gateway → ESB → Mainframe; шаг выполняется после: шаг 1 «Старый фронт входит через единый внешний вход»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 3 «Корпоративная JMS-очередь передаёт задачу в Java-контур» предлагается использовать ActiveMQ/Artemis.
**Где:** это применяется на связи ESB → ActiveMQ → Workflow engine; шаг выполняется после: шаг 2 «Шина маршрутизирует запрос в несколько старых систем»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 4 «Гарантированная очередь передаёт команду в мейнфрейм» предлагается использовать IBM MQ.
**Где:** это применяется на связи ESB → IBM MQ → Mainframe; шаг выполняется после: шаг 2 «Шина маршрутизирует запрос в несколько старых систем»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 5 «Длительный процесс хранит состояние и таймеры» предлагается использовать Движок длительного процесса.
**Где:** это применяется на связи ActiveMQ → Workflow engine → БД процесса; шаг выполняется после: шаг 3 «Корпоративная JMS-очередь передаёт задачу в Java-контур»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 6 «Ручные согласования ведутся в BPMN-процессе» предлагается использовать BPM/BPMN-движок.
**Где:** это применяется на связи Workflow engine → BPM engine → БД процесса; шаг выполняется после: шаг 5 «Длительный процесс хранит состояние и таймеры»; характер взаимодействия — без ожидания немедленного ответа.
**Почему:** Подходит, когда процесс содержит согласования, ручные бизнес-задачи, роли, требование ко времени ответа и эскалации.
...
**Что проверить перед выпуском:** Нужны роли, формы задач, требование ко времени ответа, эскалации, аудит решений, версии процесса и правила миграции активных экземпляров.

#### Шаг 7. Изменения операционной БД передаются из CDC

**Что:** в шаге 7 «Изменения операционной БД передаются из CDC» предлагается использовать Передача изменений из базы данных.
**Где:** это применяется на связи БД процесса → CDC → ETL; шаг выполняется после: шаг 6 «Ручные согласования ведутся в BPMN-процессе»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 8 «Загрузка и преобразование данных готовит слой аналитики» предлагается использовать ETL/ELT-загрузка.
**Где:** это применяется на связи CDC → ETL → Lakehouse; шаг выполняется после: шаг 7 «Изменения операционной БД передаются из CDC»; характер взаимодействия — без ожидания немедленного ответа.
**Почему:** Подходит для загрузки и преобразования данных между источниками, аналитическим хранилищем и витринами.
**Почему не другой вариант:** CDC передаёт изменения ближе к реальному времени, но не всегда заменяет сложные преобразования. Batch без ETL-правил не описывает качество и трансформации данных.
**Что проверить перед выпуском:** Нужны расписание, mapping, контроль количества записей, карантин ошибок ошибок, повторный запуск периода и журнал преобразований.

#### Шаг 9. Озеро с табличной аналитикой хранит историю

**Что:** в шаге 9 «Озеро с табличной аналитикой хранит историю» предлагается использовать Lakehouse.
**Где:** это применяется на связи ETL → Lakehouse; шаг выполняется после: шаг 8 «Загрузка и преобразование данных готовит слой аналитики»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 10 «Распределённая обработка пересчитывает большие периоды» предлагается использовать Spark.
**Где:** это применяется на связи Lakehouse → Spark → Data Warehouse; шаг выполняется после: шаг 9 «Озеро с табличной аналитикой хранит историю»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 11 «Оркестратор управляет зависимыми загрузками» предлагается использовать Airflow как оркестратор загрузок.
**Где:** это применяется на связи Spark → Airflow → dbt; шаг выполняется после: шаг 10 «Распределённая обработка пересчитывает большие периоды»; характер взаимодействия — без ожидания немедленного ответа.
**Почему:** Подходит для управления зависимыми заданиями: загрузки, проверки, преобразования, ожидания файлов и повторные запуски.
**Почему не другой вариант:** Один batch-скрипт проще, но быстро становится неуправляемым при зависимостях и требование ко времени ответа. Kafka не решает расписание и DAG-зависимости загрузок.
**Что проверить перед выпуском:** Нужны DAG, расписание, повторная попытка, требование ко времени ответа, алерты, дозагрузка исторических данных/rerun и правила обработки частично выполненных загрузок.

#### Шаг 12. Аналитические модели собирают витрины

**Что:** в шаге 12 «Аналитические модели собирают витрины» предлагается использовать dbt как слой аналитических моделей.
**Где:** это применяется на связи Data Warehouse → dbt → Data Warehouse; шаг выполняется после: шаг 11 «Оркестратор управляет зависимыми загрузками»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 13 «Событие уходит в облачную очередь AWS» предлагается использовать AWS SNS/SQS.
**Где:** это применяется на связи Workflow engine → AWS SNS/SQS; шаг выполняется после: шаг 6 «Ручные согласования ведутся в BPMN-процессе»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 14 «Событие уходит в облачный pub/sub Google» предлагается использовать Google Pub/Sub.
**Где:** это применяется на связи Workflow engine → Google Pub/Sub; шаг выполняется после: шаг 6 «Ручные согласования ведутся в BPMN-процессе»; характер взаимодействия — без ожидания немедленного ответа.
...
| 3. Корпоративная JMS-очередь передаёт задачу в Java-контур. Маршрут: ESB → ActiveMQ → Workflow engine. | ActiveMQ/Artemis | Подходит для корпоративной JMS-очереди, если в организации уже используется Java/JMS-контур и нужны стандартные enterprise-возможности очередей. | RabbitMQ обычно проще для современных task queue. Kafka лучше для event log. IBM MQ выбирают, если уже есть строгий мейнфрейм/банковский MQ-контур. | Нужны модель подтверждения, durable queues/topics, очередь ошибочных сообщений, транзакционность при необходимости и мониторинг потребителей. |
...
| 8. Загрузка и преобразование данных готовит слой аналитики. Маршрут: CDC → ETL → Lakehouse. | ETL/ELT-загрузка | Подходит для загрузки и преобразования данных между источниками, аналитическим хранилищем и витринами. | CDC передаёт изменения ближе к реальному времени, но не всегда заменяет сложные преобразования. Batch без ETL-правил не описывает качество и трансформации данных. | Нужны расписание, mapping, контроль количества записей, карантин ошибок ошибок, повторный запуск периода и журнал преобразований. |
...
| 10. Распределённая обработка пересчитывает большие периоды. Маршрут: Lakehouse → Spark → Data Warehouse. | Spark | Подходит для большой распределённой обработки данных: тяжёлые преобразования, агрегации, пересчёты истории и обработка больших файлов. | Обычный batch проще для малых объёмов. ClickHouse/аналитическое хранилище лучше для запросов по уже подготовленным данным. Spark нужен именно для распределённого вычисления. | Нужны партиционирование, checkpoint, контроль shuffle, повторный запуск, ресурсы кластера и контроль качества результата. |
| 11. Оркестратор управляет зависимыми загрузками. Маршрут: Spark → Airflow → dbt. | Airflow как оркестратор загрузок | Подходит для управления зависимыми заданиями: загрузки, проверки, преобразования, ожидания файлов и повторные запуски. | Один batch-скрипт проще, но быстро становится неуправляемым при зависимостях и требование ко времени ответа. Kafka не решает расписание и DAG-зависимости загрузок. | Нужны DAG, расписание, повторная попытка, требование ко времени ответа, алерты, дозагрузка исторических данных/rerun и правила обработки частично выполненных загрузок. |
| 12. Аналитические модели собирают витрины. Маршрут: Data Warehouse → dbt → Data Warehouse. | dbt как слой аналитических моделей | Подходит для управляемых SQL-моделей, тестов качества, lineage и прозрачной сборки аналитических витрин. | ETL-инструмент шире по загрузке данных, но dbt удобен для трансформаций внутри хранилища. Ручные SQL-скрипты хуже сопровождаются и тестируются. | Нужны модели, тесты, freshness checks, documentation, lineage, окружения и правила релиза. |
...
| Бизнес и границы | Не указано | Должен быть понятен инициатор и потребитель результата. Кто запускает процесс и кто использует результат: клиент, оператор, система, batch или регулятор? | От инициатора зависит требование ко времени ответа, ошибки, права доступа, UX и требования к статусам. | Укажите actor, channel, expected response и способ получения финального результата. |
...
| аналитическое хранилище и витрины | Блокирует выпуск | аналитическое хранилище не должен находиться в основной поток. Останавливает ли аналитика операционный процесс? | аналитическое хранилище обычно не рассчитан на операционный требование ко времени ответа. | Выгружайте через CDC/ETL после фиксации операции. |
...
2. **аналитическое хранилище или аналитика находятся в операционном основной поток.** — Шаг 9 «Озеро с табличной аналитикой хранит историю» → Lakehouse.
   - Что может пойти не так: Аналитическое хранилище обычно не обеспечивает операционный требование ко времени ответа: его деградация не должна останавливать основной бизнес-процесс.
...
- **Что:** аналитическое хранилище или аналитика находятся в операционном основной поток.
  ...
  **Почему важно:** Аналитическое хранилище обычно не обеспечивает операционный требование ко времени ответа: его деградация не должна останавливать основной бизнес-процесс.
