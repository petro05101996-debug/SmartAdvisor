...
Обычные backend/unit regression-тесты не требуют браузера:
...
Полный UI-regression прогон вынесен отдельно и требует Playwright + Chromium только в dev/CI окружении:

```bash
python -m pip install -r requirements-dev.txt
python -m playwright install chromium
python -m pytest -q tests/ui -m browser
```

Если Playwright или Chromium не установлены, browser-тесты делают `pytest.skip` и не ломают обычную Python-сборку. Production Dockerfile не должен устанавливать браузер.

Все тесты, включая browser suite при установленном Chromium:
...
В репозитории есть GitHub Actions workflow `.github/workflows/ci.yml` с двумя независимыми job:

- **backend/unit regression**: устанавливает Python 3.11, dev-зависимости, компилирует приложение и запускает `python -m pytest -q -m "not browser"`;
- **UI browser regression**: дополнительно устанавливает Playwright Chromium и запускает `python -m pytest -q tests/ui -m browser`.

Так обычная сборка не зависит от браузера, а полный UI-click-through выполняется отдельным CI job.

## Как пользоваться

### Для новичка

Используйте новый простой мастер:

1. Нажмите **“Начать проектирование”**.
2. Выберите сценарий карточкой или нажмите **“Не знаю, помогите выбрать”** и ответьте на мини-опрос.
3. Ответьте на простые бизнес-вопросы: кто запускает процесс, что должно произойти, есть ли деньги/ПДн/регуляторика, нужен ли быстрый ответ.
4. Добавьте системы карточками: название, роль, критичность, владелец и канал.
5. Выберите шаблон цепочки или соберите процесс шагами.
6. Посмотрите **“Проверку перед отчётом”**: незаполненные пункты не блокируют генерацию, а попадут в риски и вопросы.
7. Нажмите **“Сформировать отчёт”**.

### Для системного аналитика

Начните с простого мастера, затем при необходимости нажмите **“Расширенный режим”** и уточните экспертные поля и матрицы:
...
- rollout, recovery, capacity и export bundle.
