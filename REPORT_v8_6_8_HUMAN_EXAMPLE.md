...
**Что:** в шаге 1 «Клиент отправляет заявку через внешний вход» предлагается использовать API Gateway.
**Где:** это применяется на связи Мобильное приложение → API Gateway → Сервис страховой заявки; шаг выполняется после: начало процесса или внешний запуск; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 2 «Пользователь подтверждает доступ через единую авторизацию» предлагается использовать OAuth2/OIDC.
**Где:** это применяется на связи Мобильное приложение → OAuth/OIDC → API Gateway; шаг выполняется после: шаг 1 «Клиент отправляет заявку через внешний вход»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 3 «Процесс выплаты долгий и хранит состояния» предлагается использовать Движок длительного процесса.
**Где:** это применяется на связи Сервис страховой заявки → Workflow engine → БД процесса; шаг выполняется после: шаг 2 «Пользователь подтверждает доступ через единую авторизацию»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 4 «Сервис получает полис из старого core по WSDL» предлагается использовать SOAP.
**Где:** это применяется на связи Workflow engine → Сервис страховой заявки → Legacy core; шаг выполняется после: шаг 3 «Процесс выплаты долгий и хранит состояния»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 5 «Документы заявки сохраняются отдельно от БД» предлагается использовать Объектное хранилище.
**Где:** это применяется на связи Сервис страховой заявки → Object Storage; шаг выполняется после: шаг 3 «Процесс выплаты долгий и хранит состояния»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 6 «OCR получает фоновую задачу» предлагается использовать RabbitMQ.
**Где:** это применяется на связи Сервис страховой заявки → RabbitMQ → Сервис страховой заявки; шаг выполняется после: шаг 5 «Документы заявки сохраняются отдельно от БД»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 7 «Антифрод вызывается синхронно и возвращает решение» предлагается использовать REST API.
**Где:** это применяется на связи Сервис страховой заявки → REST антифрод; шаг выполняется после: шаг 4 «Сервис получает полис из старого core по WSDL»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 8 «Нельзя одновременно менять одну выплату» предлагается использовать Redis как распределённая блокировка.
**Где:** это применяется на связи Сервис страховой заявки → Redis lock → Сервис страховой заявки; шаг выполняется после: шаг 7 «Антифрод вызывается синхронно и возвращает решение»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 9 «Итоговый статус выплаты сохраняется» предлагается использовать Основная база данных.
**Где:** это применяется на связи Сервис страховой заявки → БД процесса; шаг выполняется после: шаг 8 «Нельзя одновременно менять одну выплату»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 10 «Профиль выплаты часто читается из кэша» предлагается использовать Redis как кэш.
**Где:** это применяется на связи БД процесса → Redis cache → Сервис страховой заявки; шаг выполняется после: шаг 9 «Итоговый статус выплаты сохраняется»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 11 «Поиск заявок идёт по индексу» предлагается использовать Поисковый индекс.
**Где:** это применяется на связи БД процесса → Search index → Сервис страховой заявки; шаг выполняется после: шаг 9 «Итоговый статус выплаты сохраняется»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 12 «Платёжный провайдер присылает поздний результат» предлагается использовать Обратный вызов.
**Где:** это применяется на связи Обратный вызов оплаты → API Gateway → Сервис страховой заявки; шаг выполняется после: шаг 9 «Итоговый статус выплаты сохраняется»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 13 «Партнёр доставки сам присылает статус документа» предлагается использовать Входящий веб-вызов.
**Где:** это применяется на связи Входящий веб-вызов доставки → API Gateway → Сервис страховой заявки; шаг выполняется после: шаг 9 «Итоговый статус выплаты сохраняется»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 14 «Медицинский партнёр получает защищённый файловый пакет» предлагается использовать SFTP.
**Где:** это применяется на связи Сервис страховой заявки → SFTP медпартнёра; шаг выполняется после: шаг 9 «Итоговый статус выплаты сохраняется»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 15 «События выплаты читают уведомления, аудит и аналитика» предлагается использовать Kafka.
**Где:** это применяется на связи Сервис страховой заявки → Kafka; шаг выполняется после: шаг 9 «Итоговый статус выплаты сохраняется»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 16 «Изменения передаются в аналитическое хранилище» предлагается использовать Передача изменений из базы данных.
**Где:** это применяется на связи БД процесса → аналитическое хранилище; шаг выполняется после: шаг 9 «Итоговый статус выплаты сохраняется»; характер взаимодействия — без ожидания немедленного ответа.
...
**Что:** в шаге 17 «Секреты партнёров и ключи подписи хранятся защищённо» предлагается использовать Vault/KMS для секретов и ключей.
**Где:** это применяется на связи Сервис страховой заявки → Vault/KMS → REST антифрод; шаг выполняется после: шаг 7 «Антифрод вызывается синхронно и возвращает решение»; характер взаимодействия — с ожиданием ответа сразу.
...
**Что:** в шаге 18 «Наблюдаемость показывает зависшие выплаты и ошибки партнёров» предлагается использовать Наблюдаемость.
**Где:** это применяется на связи Сервис страховой заявки → Observability; шаг выполняется после: шаг 12 «Платёжный провайдер присылает поздний результат», шаг 13 «Партнёр доставки сам присылает статус документа»; характер взаимодействия — с ожиданием ответа сразу.
...
| 2. Пользователь подтверждает доступ через единую авторизацию. Маршрут: Мобильное приложение → OAuth/OIDC → API Gateway. | OAuth2/OIDC | Подходит для единой авторизации пользователей, сервисов или партнёров с токенами, scopes и централизованной проверкой доступа. | Самописная авторизация опаснее и сложнее в сопровождении. API Gateway может проверять токен, но источник идентичности и протокол всё равно нужны отдельно. | Нужны scopes/claims, срок жизни токена, refresh-правила, аудит входа, ротация ключей и проверка прав на уровне действий. |
| 3. Процесс выплаты долгий и хранит состояния. Маршрут: Сервис страховой заявки → Workflow engine → БД процесса. | Движок длительного процесса | Подходит для долгого процесса с состояниями, таймерами, ожиданием внешних результатов и компенсационными действиями. | Простая БД со статусами может хватить для короткого процесса, но становится хрупкой при таймерах, ожиданиях, повторная попытка и компенсациях. Kafka хранит события, но не заменяет orchestration state. | Нужны модель состояний, таймеры, компенсации, история процесса, идемпотентность команд и правила ручного вмешательства. |
...
| 16. Изменения передаются в аналитическое хранилище. Маршрут: БД процесса → аналитическое хранилище → аналитическое хранилище. | Передача изменений из базы данных | Подходит для передачи зафиксированных изменений из операционной БД в аналитику или витрины без участия основного пользовательского потока. | Прямые записи из сервиса в аналитическое хранилище создают связанность и риск замедлить бизнес-операцию. Batch проще, но даёт большую задержку. Kafka из приложения требует outbox-дисциплины. | Нужны контроль позиции чтения, контроль отставания, совместимость схем, повторная синхронизация и сверка полноты. |
...
| medium | Внешние системы | Какие лимит запросовs у внешних систем и что делать при 429/лимите? | Без лимитов нельзя оценить пиковую нагрузку и backpressure. |
...
| аналитическое хранилище и витрины | Не указано | Read-model должен иметь freshness contract. Насколько данные могут отставать от источника? | Потребитель должен понимать, что читает проекцию, а не источник истины. | Показывайте lastUpdatedAt, отставание обработки и требование ко времени ответа свежести. |
...
4. **Входящий входящий веб-вызов должен проходить проверку подлинности.** — Затронуто мест: 2. Шаг 12 «Платёжный провайдер присылает поздний результат»; Шаг 13 «Партнёр доставки сам присылает статус документа». Затронуто мест: 2
...
6. **Повторы в синхронной цепочке усиливают друг друга.** — «Антифрод вызывается синхронно и возвращает решение» → «Нельзя одновременно менять одну выплату» → «Итоговый статус выплаты сохраняется» → «Поиск заявок идёт по индексу».
   ...
   - Как должно обрабатываться: Задайте единый бюджет повторов на весь запрос (global deadline), предохранитель внешнего вызова на каждом звене и экспоненциальный увеличивающаяся пауза между повторами с случайный разброс паузы; не повторяйте вызовы, которые уже не успеют уложиться в требование ко времени ответа.
...
- **Что:** Входящий входящий веб-вызов должен проходить проверку подлинности. (затронуто мест: 2)
  ...
  **Почему важно:** Входящий входящий веб-вызов является публичной точкой входа: без проверки подписи любой внешний отправитель может подделать бизнес-событие обычным POST-запросом.
...
- **Что:** Повторы в синхронной цепочке усиливают друг друга.
  ...
  **Что нужно сделать:** Задайте единый бюджет повторов на весь запрос (global deadline), предохранитель внешнего вызова на каждом звене и экспоненциальный увеличивающаяся пауза между повторами с случайный разброс паузы; не повторяйте вызовы, которые уже не успеют уложиться в требование ко времени ответа.
...
20. Регресс на «Дочерний вызов может ждать дольше, чем родительский шаг.»: Таймауты должны строго убывать вниз по цепочке: дочерний timeout должен быть меньше родительского с учётом сетевых накладных; общий бюджет времени распределяйте от требование ко времени ответа сверху вниз.
