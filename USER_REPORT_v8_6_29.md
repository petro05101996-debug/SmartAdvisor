# Архитектурный разбор: Проверка пользовательского отчёта v8.6.29

## Короткий человеческий вывод

**Итог:** УСЛОВНО ГОТОВО: закрыть высокие риски. **Архитектурная готовность:** 5.8/10. **Готовность к промышленному запуску:** нельзя выпускать без закрытия блокеров.

**Полнота вводных:** 30%. **Надёжность рекомендаций:** низкая.

**Масштаб процесса:** 4 взаимодействий, из них 4 в основной цепочке и 0 сквозных контролей. Участников: 6.

**Бизнес-цель:** Проверка отчёта после замечаний пользователя
...
| Средний | В блокирующих шагах не задан таймаут. | 1 «Система-инициатор сохраняет результат в Сервис процесса», 3 «Сервис процесса передаёт данные в Внешняя система / партнёр» | Задайте таймаут каждому блокирующему вызову и распределите бюджет времени от целевого времени ответа сверху вниз. |
| информационные | Для служебных таблиц не описана политика роста и очистки. | outbox + журнал шагов | Добавьте партиционирование по времени, архивацию или перенос в холодное хранилище, а также очистку опубликованных записей таблицы исходящих сообщений; контролируйте размер таблиц и время запросов к ним. |
...
- **Шаг 1 «Система-инициатор сохраняет результат в Сервис процесса»**: Шаг описан как сохранение результата, но получатель не похож на хранилище данных. Измените действие на «передаёт данные» либо выберите БД/хранилище состояния как получателя.
...
### Шаг 1. Система-инициатор сохраняет результат в Сервис процесса

**Что:** шаг 1 — «Система-инициатор сохраняет результат в Сервис процесса». Основной способ взаимодействия: сначала исправить схему.
...
**Где:** связь идёт от «Сервис процесса» к «Старый контур». Исполнитель: «Сервис процесса». Выполняется после: шаг 1 «Система-инициатор сохраняет результат в Сервис процесса».
...
| Целостность данных | Требует проверки | Для процесса предусмотрена сверка. |
...
|---|---|---|---|
| 1. Система-инициатор сохраняет результат в Сервис процесса | Система-инициатор → Сервис процесса. Исполнитель: Система-инициатор | сначала исправить схему | Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека. |
...
**Затронутые места:** 1 «Система-инициатор сохраняет результат в Сервис процесса», 3 «Сервис процесса передаёт данные в Внешняя система / партнёр».
...
|---|---|---|---|
| 1 | Система-инициатор → Сервис процесса | Система-инициатор сохраняет результат в Сервис процесса | шаг завершён, результат отражён в статусе процесса или журнале шагов |
...
- Отказ шага 1 «Система-инициатор сохраняет результат в Сервис процесса»: таймаут/5xx — процесс не зависает, статус и алерт корректны.
...
flowchart LR
 ...
 N1 -->|"1. Система-инициатор сохраняет результат в Сервис… / сначала исправить схему"| N2
...
sequenceDiagram
 ...
 N1->>N2: 1. Система-инициатор сохраняет результат в Сервис… / сначала исправить схему
...
Матрица деталей: применимо инвариантов из каталога v7.1 — 103 из 125; блокируют выпуск — 10, требуют внимания — 31, нужно уточнить — 69, уже выглядит закрытым — 8.
...
| Асинхронность и брокеры | Требует проверки | Partition key должен соответствовать требованию порядка. | Без правильного ключа события одной сущности могут прийти out-of-order. | Используйте entityId/идентификатор агрегата как ключ партиционирования там, где нужен порядок внутри одной сущности. |
