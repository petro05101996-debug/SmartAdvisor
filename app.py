...
APP_VERSION = getattr(ui, 'APP_VERSION', '8.6.72-production-ready')
SERVICE_NAME = 'SmartAdvisor'
...
def _diagnostics_payload(deep=False):
    """Small production diagnostics for platform readiness probes and support."""
    checks = {
    ...
    except Exception as exc:  # pragma: no cover - defensive readiness output
    ...
    payload = {
        ...
        'version': APP_VERSION,
        'checks': checks,
        'limits': {'max_post_bytes': MAX_POST},
        'features': [
            'process_map',
            'architecture_review',
            'project_package',
    ...
    if deep:
        payload['learning_case_count'] = len(list_cases())
    return payload
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_HEAD(self):
        ...
        if path in ('/glossary', '/glossary/', '/glossary.html', '/progress', '/progress/', '/progress.html', '/reports', '/reports/', '/reports.html'):
            return self._send_head(200)
    ...
    def do_GET(self):
        ...
        if path in ('/glossary', '/glossary/', '/glossary.html'):
            return self._send(200, ui.glossary_page())
        if path in ('/progress', '/progress/', '/progress.html'):
            return self._send(200, ui.progress_page())
        if path in ('/reports', '/reports/', '/reports.html'):
            return self._send(200, ui.reports_page())
        ...
        if path in HEALTH_PATHS:
            deep = path in ('/readyz', '/healthz')
            payload = _diagnostics_payload(deep=deep)
            return self._send(200 if payload.get('ok') else 503, json.dumps(payload, ensure_ascii=False, default=str), 'application/json')
        if path in ('/api/version', '/version'):
            return self._send(200, json.dumps({'ok': True, 'service': SERVICE_NAME, 'version': APP_VERSION}, ensure_ascii=False), 'application/json')
        if path == '/api/diagnostics':
            return self._send(200, json.dumps(_diagnostics_payload(deep=True), ensure_ascii=False, default=str), 'application/json')
        m = RUN_RE.match(path)
        if m:
            res = load_run(m.group(1))
            if not res:
