...
if __name__=='__main__':
    combos=list(product(STARTS,ACTS,TIMINGS,RESULTS,SYSTEMS))
    ...
    for combo in combos:
        ...
        if module_bad: break
    print('SINGLE_MODULE',len(combos)*len(MODULES),'bad',len(module_bad),'ui_ref_issues',module_ui_issues,'colors',dict(module_stats),'top_rules',module_rules.most_common(12))
    ...
    for combo in seeds:
        ...
        if pair_bad: break
    print('PAIR_MODULE_SEEDS',len(seeds)*45,'bad',len(pair_bad),'ui_ref_issues',pair_ui,'colors',dict(pair_stats))

    hard=('unknown','validate','unknown','unknown','unknown')
    subset_stats=Counter(); subset_bad=[]; subset_ui=0
    for r in range(0,len(MODULES)+1):
        ...
        if subset_bad: break
    print('ALL_SUBSETS_HARD',2**len(MODULES),'bad',len(subset_bad),'ui_ref_issues',subset_ui,'colors',dict(subset_stats))
