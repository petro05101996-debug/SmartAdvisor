...
SYNC_CHANNELS = {'rest', 'soap', 'grpc', 'api_gateway', 'esb', 'db', 'redis_cache', 'redis_lock', 'search'}
BROKER_CHANNELS = {'kafka', 'queue', 'rabbitmq', 'redis_streams', 'redis_queue'}
ASYNC_CHANNELS = BROKER_CHANNELS | {'webhook', 'callback', 'file', 'batch', 'sftp', 'object_storage', 'cdc'}
...
CHANNEL_RU = {
    'rest': 'синхронный вызов по REST API',
    'grpc': 'быстрый внутренний вызов по gRPC',
    'soap': 'вызов старой или внешней системы по SOAP',
    'api_gateway': 'единый внешний вход через API Gateway',
    'esb': 'интеграционная шина ESB',
    'db': 'запись или чтение из основной базы данных',
    'redis_cache': 'кэш Redis для ускорения чтения',
    'redis_lock': 'распределённая блокировка Redis',
    'search': 'поисковый индекс',
    'kafka': 'поток событий Kafka',
    'rabbitmq': 'очередь задач RabbitMQ',
    'redis_streams': 'поток Redis Streams',
    'redis_queue': 'короткая очередь задач Redis',
    'queue': 'очередь сообщений, брокер пока не выбран',
    'webhook': 'входящий веб-вызов от внешней системы',
    'callback': 'обратный вызов от внешней системы',
    'sftp': 'файловый обмен по SFTP',
    'file': 'файловая передача',
    'object_storage': 'объектное хранилище для файлов',
    'batch': 'пакетная обработка по расписанию',
    'cdc': 'передача изменений из базы данных в аналитику',
}
...
def _step_failure_handling(step, system_role='internal'):
    ...
    if ch in ('rest', 'soap', 'grpc', 'api_gateway', 'esb'):
    ...
    if ch == 'db':
        ...
        return ' '.join(parts)
    if ch in ('file', 'batch', 'sftp', 'object_storage'):
