...
def normalize(payload):
    ...
    for i, st in enumerate(payload.get('steps') or [], 1):
        ...
        steps.append({
            ...
            # v8.6.29: сохраняем семантику связи из UI. Без этих полей отчёт
            # вынужден угадывать смысл шага только по названию/каналу и снова
            # путает запрос данных, публикацию события, сохранение и аналитику.
            'interaction_action': _s(st.get('interaction_action')),
            'interaction_timing': _s(st.get('interaction_timing')),
            'interaction_result': _s(st.get('interaction_result')),
            'primary_channel': _s(st.get('primary_channel')),
            'component_type': _s(st.get('component_type')),
            'dependency_basis': _s(st.get('dependency_basis')),
        })
...
def _is_actual_entity_write(step, systems):
    name = _low(step.get('name'))
    target = _s(step.get('target_system'))
    role = (systems.get(target) or {}).get('role', '')
    action = _low(step.get('interaction_action'))
    channel = _low(step.get('channel'))
    if action in ('save', 'update_status'):
        return role in ('db', 'analytics') or any(x in _low(target) for x in ['бд', 'база', 'хранилище состояния', 'статус', 'ledger', 'журнал'])
    if channel == 'db' or role == 'db':
        return True
    if any(x in name for x in ['сохраняет', 'обновляет статус', 'изменяет состояние', 'записывает']):
        return role in ('db', 'analytics') or any(x in _low(target) for x in ['бд', 'база', 'хранилище', 'журнал'])
    return False
...
def build_graph(model):
    ...
    model['graph'] = {
        ...
        # v8.6.29: не считаем инициатора писателем только потому, что он отправил
        # запрос в сервис процесса. Писателем считаем шаги, где явно есть запись
        # в хранилище/статус/агрегат. Иначе отчёт ложно ругается на multiple writers.
        'writers': sorted({s['system'] for s in steps if s['writes_entity'] and (meta.get('money') == 'direct' or _is_actual_entity_write(s, systems))}),
...
def _find_cycle(model):
    """Возвращает цикл зависимостей шагов или None.

    В v8 появилась поддержка fan-in: у шага может быть несколько родителей
    в поле ``deps``. Старый алгоритм смотрел только ``depends_on``
    (первого родителя) и пропускал циклы, которые проходили через второго
    или последующего родителя. Для ядра это критично: схема перестаёт быть
    DAG, но анализ всё равно продолжает строить отчёт как будто всё валидно.
    Поэтому проверяем полный ориентированный граф: шаг -> все его зависимости.
    """
    by_order = {s['order']: s for s in model['steps']}
    visiting = set()
    visited = set()
    stack = []
    ...
    def dfs(order):
        if order in visiting:
            try:
                return stack[stack.index(order):] + [order]
            except ValueError:
                return [order, order]
        if order in visited:
            return None
        visiting.add(order)
        stack.append(order)
        step = by_order.get(order) or {}
        for dep in step.get('deps') or []:
            if dep not in by_order:
                continue
            cycle = dfs(dep)
            if cycle:
                return cycle
...
# ---------------------------------------------------------------------------
# v8.6.30: external blocking должен означать исходящий блокирующий вызов во
# внешнюю систему, а не входящий запрос от внешнего инициатора к нашему сервису.
# Раньше шаг «Система-инициатор -> Сервис процесса» ошибочно считался блокировкой
# на внешней зависимости, потому что executor имел роль external.
_PREV_BUILD_GRAPH_V8630 = build_graph

def _v8630_system_role(model, name):
    return ((model.get('systems') or {}).get(name) or {}).get('role', '')

def _v8630_is_outbound_external_blocking(model, step):
    if not step.get('blocking'):
        return False
    src = step.get('source_system') or ''
    tgt = step.get('target_system') or ''
    system = step.get('system') or ''
    src_role = _v8630_system_role(model, src)
    tgt_role = _v8630_system_role(model, tgt)
    sys_role = _v8630_system_role(model, system)
    # Наша/внутренняя система ждёт внешний target.
    if tgt_role in ('external', 'legacy') and src_role not in ('external', 'legacy'):
        return True
    # Если target не заполнен, но executor является внешней зависимостью, это тоже внешний вызов.
    if sys_role in ('external', 'legacy') and src_role not in ('external', 'legacy') and (not tgt or tgt == system):
        return True
    return False

def build_graph(model):
    model = _PREV_BUILD_GRAPH_V8630(model)
    model['graph']['external_blocking'] = [s for s in model.get('steps', []) if _v8630_is_outbound_external_blocking(model, s)]
    return model

# Заменяем функцию правила external_blocking, чтобы where показывал фактического получателя.
def _r_external_blocking_v8630(model):
    out = []
    meta = model['meta']
    for s in model['graph']['external_blocking']:
        sev = 'critical' if (meta['customer_visible'] and 0 < meta['sla_ms'] <= 1000) else 'high'
        target = s.get('target_system') or s.get('system') or 'внешняя система'
        out.append(F(sev, 'Процесс блокируется на вызове внешней системы.',
                     f"Шаг {s['order']} «{s['name']}» → {target}",
                     'Внешняя система находится вне вашего контроля: её деградация напрямую ухудшает ваш SLA и может исчерпать пул рабочих потоков.',
                     'Настройте таймаут, предохранитель внешнего вызова и запасной сценарий-ответ; если бизнес-сценарий позволяет, переведите шаг в асинхронную обработку через очередь с компенсацией.'))
    return out

for _r in RULES:
    if _r.get('id') == 'external_blocking':
        _r['fn'] = _r_external_blocking_v8630

# ---------------------------------------------------------------------------
# v8.6.33: real-report audit fixes.
# Цель: не считать исходящий запрос к партнёру входящим webhook, не ловить
# ложный Outbox на внешнем инициаторе и показывать места рисков без дублей.

def _v8633_role(model, system_name):
    return ((model.get('systems') or {}).get(system_name or '') or {}).get('role', '')


def _v8633_is_inbound_web_result(model, step):
    src = step.get('source_system') or ''
    tgt = step.get('target_system') or ''
    name = _low(step.get('name'))
    src_role = _v8633_role(model, src)
    tgt_role = _v8633_role(model, tgt)
    if step.get('channel') not in ('webhook', 'callback'):
        return False
    # Настоящий входящий результат: внешний/legacy источник приходит в наш контур.
    if src_role in ('external', 'legacy') and tgt_role not in ('external', 'legacy'):
        return True
    # Дополнительная защита для старых payload-ов без ролей.
    if re.search(r'принять|получить|входящ|статус|результат|callback|обратн', name) and src_role in ('external', 'legacy'):
        return True
    return False


def _r_inbound_security_v8633(model):
    out = []
    for s in model['steps']:
        if not _v8633_is_inbound_web_result(model, s):
            continue
        blob = _low(s['name']) + ' ' + _low(s['data_in'])
        verified = bool(re.search(r'подпис|sign|hmac|jwt|mtls|mTLS', blob))
        sev = 'info' if verified else 'high'
        why = ('Входящий веб-вызов является публичной точкой входа: без проверки подписи любой внешний отправитель '
               'может подделать бизнес-событие обычным POST-запросом.')
        fix = ('Проверяйте HMAC или подпись провайдера до любой бизнес-обработки; '
               'секрет храните в защищённом хранилище и предусмотрите его ротацию.')
        if verified:
            why = 'Подпись проверяется, поэтому этот контроль нужно явно зафиксировать в требованиях и тестах.'
            fix = 'Добавьте негативный тест: запрос с неверной подписью отклоняется до запуска бизнес-логики.'
        out.append(F(sev, 'Входящий веб-вызов должен проходить проверку подлинности.',
                     f"Шаг {s['order']} «{s['name']}»", why, fix))
    return out


def _r_callback_inbox_v8633(model):
    out = []
    for s in model['steps']:
        if _v8633_is_inbound_web_result(model, s) and s['idempotency'] == 'none':
            out.append(F('high', 'Входящий веб-вызов принимается без защиты от дублей.',
                         f"Шаг {s['order']} «{s['name']}»",
                         'Провайдеры часто доставляют один и тот же результат повторно. Без таблицы входящих сообщений один и тот же ответ может быть обработан дважды.',
                         'Добавьте таблицу входящих сообщений с уникальным providerEventId; бизнес-обработку запускайте только после успешной вставки ключа дедупликации.'))
    return out


def _r_dual_write_v8633(model):
    by_system = {}
    ...
    out = []
    for sysname, ss in by_system.items():
        # Внешний инициатор, который только публикует событие в наш контур, не должен
        # автоматически считаться владельцем нашей БД и нарушителем Outbox.
        if _v8633_role(model, sysname) in ('external', 'legacy'):
            continue
        writes = any(_is_actual_entity_write(x, systems) for x in ss)
        publishes = [x for x in ss if x['channel'] in BROKER_CHANNELS and not _is_consumption(model, x)]
        mentions_outbox = any('outbox' in _low(x['compensation']) + _low(x['data_out']) + _low(x.get('primary_channel')) for x in ss)
        if writes and publishes and not mentions_outbox:
            places = ', '.join(f"шаг {x['order']} «{x['name']}»" for x in publishes)
            out.append(F('high', 'Система одновременно пишет в БД и публикует событие без таблицы исходящих сообщений.',
                         f"{sysname}: {places}",
                         'Запись в БД и публикация события являются двумя несвязанными операциями: при сбое между ними событие может потеряться или появиться без соответствующей записи.',
                         'Используйте транзакционную таблицу исходящих сообщений: событие записывается в той же транзакции, что и агрегат, а отдельный публикатор читает таблицу исходящих сообщений и публикует событие с повторными попытками.'))
    return out


for _r in RULES:
    if _r.get('id') == 'inbound_security':
        _r['fn'] = _r_inbound_security_v8633
    elif _r.get('id') == 'callback_inbox':
        _r['fn'] = _r_callback_inbox_v8633
    elif _r.get('id') == 'dual_write':
        _r['fn'] = _r_dual_write_v8633


_PREV_GROUP_FINDINGS_V8633 = group_findings

def group_findings(findings, max_items_preview=12):
    groups = _PREV_GROUP_FINDINGS_V8633(findings, max_items_preview=max_items_preview)
    for g in groups:
        affected = []
        seen = set()
        for w in g.get('affected') or []:
            w = str(w).strip()
            ...
            affected.append(w)
        g['affected'] = affected
        g['count'] = len(affected) or int(g.get('count') or 0)
        if g['count'] <= 1:
            g['where'] = affected[0] if affected else (g.get('where') or '—')
            g['where_summary'] = g['where']
        else:
            preview = '; '.join(affected[:max_items_preview])
            tail = '' if len(affected) <= max_items_preview else f'; ещё {len(affected) - max_items_preview}'
            g['where'] = f'Затронуто мест: {g["count"]}. {preview}{tail}'
            g['where_summary'] = g['where']
    return groups

# v8.6.33b: совместимость старых payload-ов без source/target.
def _v8633_is_inbound_web_result(model, step):
    if step.get('channel') not in ('webhook', 'callback'):
        return False
    src = step.get('source_system') or ''
    ...
    if src_role in ('external', 'legacy') and tgt_role not in ('external', 'legacy'):
        return True
    if re.search(r'принять|получить|входящ|статус|результат|callback|webhook|обратн', name) and (not tgt_role or tgt_role not in ('external', 'legacy')):
        return True
    return False
...
def _r_dual_write_v8633(model):
    ...
    for sysname, ss in by_system.items():
        if _v8633_role(model, sysname) in ('external', 'legacy'):
            continue
        writes = any(x.get('writes_entity') or _is_actual_entity_write(x, systems) for x in ss)
        publishes = [x for x in ss if x['channel'] in BROKER_CHANNELS and not _is_consumption(model, x)]
        mentions_outbox = any('outbox' in (_low(x.get('compensation')) + _low(x.get('data_out')) + _low(x.get('primary_channel'))) for x in ss)
        if writes and publishes and not mentions_outbox:
            places = ', '.join(f"шаг {x['order']} «{x['name']}»" for x in publishes)
            out.append(F('high', 'Система одновременно пишет в БД и публикует событие без таблицы исходящих сообщений.',
                         f"{sysname}: {places}",
...
# ---------------------------------------------------------------------------
# v8.6.36: semantic endpoint fixes for broker consumers and outbound targets.
# Earlier rules mostly looked at ``step.system``. That works for old payloads
# where a Kafka publish/consume step is executed by a broker node itself, but
# it is wrong for the current UI model where a relation has source_system,
# executor system and target_system. Example:
#   Producer --kafka--> Kafka, Consumer reads from Kafka
# The consumer step has source_system=Kafka and system=Consumer, so it must be
# treated as consumption, not as another publication by Consumer.

def _v8636_role(model, system_name):
    return ((model.get('systems') or {}).get(system_name or '') or {}).get('role', '')


def _v8636_is_broker_endpoint(model, system_name):
    role = _v8636_role(model, system_name)
    text = _low(system_name)
    return role == 'broker' or bool(re.search(r'kafka|rabbit|broker|очеред|топик|stream|mq\b|pulsar|nats', text))


def _v8636_is_inbound_external(model, step):
    src = step.get('source_system') or ''
    tgt = step.get('target_system') or ''
    sysname = step.get('system') or ''
    src_role = _v8636_role(model, src)
    tgt_role = _v8636_role(model, tgt)
    sys_role = _v8636_role(model, sysname)
    return bool(src and tgt and src_role in ('external', 'legacy') and tgt_role not in ('external', 'legacy') and (sysname == src or sys_role in ('external', 'legacy')))


def _v8636_outbound_external_or_legacy(model, step):
    src = step.get('source_system') or ''
    tgt = step.get('target_system') or ''
    sysname = step.get('system') or ''
    src_role = _v8636_role(model, src)
    tgt_role = _v8636_role(model, tgt)
    sys_role = _v8636_role(model, sysname)
    if tgt and tgt_role in ('external', 'legacy') and src_role not in ('external', 'legacy'):
        return True
    if sys_role in ('external', 'legacy') and src_role not in ('external', 'legacy') and (not tgt or tgt == sysname):
        return True
    return False


def _v8636_dependency_endpoint(model, step):
    """Return the actual dependency endpoint for risk/rate-limit rules.

    Prefer target_system for outbound calls because current UI stores the
    executing service in ``system`` and the called dependency in ``target_system``.
    Avoid flagging an external initiator that merely calls our internal endpoint.
    """
    src = step.get('source_system') or ''
    tgt = step.get('target_system') or ''
    sysname = step.get('system') or ''
    if _v8636_is_inbound_external(model, step):
        return ''
    if tgt and tgt in (model.get('systems') or {}) and tgt != src:
        return tgt
    return sysname


# Override global helper used by multiple existing rules.
def _is_consumption(model, s):
    """True when a broker-channel step reads from a broker.

    Supports both old payloads (parent step executed by broker) and new semantic
    payloads (source_system is Kafka/Rabbit/etc., system is the consumer).
    """
    if s.get('channel') not in BROKER_CHANNELS:
        return False
    if _v8636_is_broker_endpoint(model, s.get('source_system')) and not _v8636_is_broker_endpoint(model, s.get('target_system')):
        return True
    if any(_v8636_is_broker_endpoint(model, p.get('system'))
           for p in (model['graph']['by_order'].get(d) for d in s.get('deps') or []) if p):
        return True
    # Extra compatibility for steps that depend on a publish-to-broker step and
    # are explicitly named as consumer/read operation.
    name_blob = _low(s.get('name')) + ' ' + _low(s.get('data_in'))
    if re.search(r'consume|consumer|read from|listen|poll|получ|прочит|читает|потреб', name_blob):
        if any(_v8636_is_broker_endpoint(model, p.get('target_system')) or _v8636_is_broker_endpoint(model, p.get('system'))
               for p in (model['graph']['by_order'].get(d) for d in s.get('deps') or []) if p):
            return True
    return False


# Re-wrap graph so step['external'] also means an outbound external/legacy
# dependency, not only "executor system has role external".
_PREV_BUILD_GRAPH_V8636 = build_graph

def build_graph(model):
    model = _PREV_BUILD_GRAPH_V8636(model)
    for s in model.get('steps', []):
        s['external'] = bool(s.get('external')) or _v8636_outbound_external_or_legacy(model, s)
    model['graph']['external_blocking'] = [s for s in model.get('steps', []) if s.get('blocking') and _v8636_outbound_external_or_legacy(model, s)]
    return model


def _r_unstable_v8636(model):
    out = []
    hit = set()
    for s in model['steps']:
        dep = _v8636_dependency_endpoint(model, s)
        if not dep or dep not in model['systems']:
            continue
        st = model['systems'][dep].get('stability')
        key = (s['order'], dep, st)
        if key in hit:
            continue
        hit.add(key)
        if st == 'unstable' and s['blocking']:
            out.append(F('high', 'Процесс блокируется на нестабильной системе.',
                         f"Шаг {s['order']} «{s['name']}» → {dep}",
                         'Система заявлена как нестабильная, поэтому её таймауты и деградация станут вашими проблемами.',
                         'Добавьте circuit breaker, bulkhead с отдельным пулом и сценарий деградации: можно показать последний известный результат или поставить задачу в очередь.'))
        if st == 'limited':
            out.append(F('medium', 'У зависимости есть rate limit.',
                         f"Шаг {s['order']} «{s['name']}» → {dep}",
                         'Лимиты провайдера могут превратить пик нагрузки в шторм ошибок 429.',
...
def _r_capacity_v8636(model):
    ...
    for s in model['steps']:
        ...
        if not dep or dep not in model['systems']:
            continue
        sysd = model['systems'][dep]
        limit = sysd.get('rate_limit_rps', 0)
        if limit and meta['peak_rps'] > limit and dep not in hit:
            hit.add(dep)
            out.append(F('critical', 'Пиковая нагрузка превышает rate limit зависимости',
                         f"{dep}: пик {meta['peak_rps']} RPS > лимит {limit} RPS",
                         'В пик каждый запрос сверх лимита получит 429/отказ — это шторм ошибок ровно в момент максимума бизнеса.',
                         'Очередь выравнивания нагрузки перед зависимостью; клиентский rate limiter; backoff с jitter; договориться о повышении лимита или деградация (кэш/отложенная проверка).'))
    return out


def _r_blocking_in_async_v8636(model):
    out, byo = [], model['graph']['by_order']
    for s in model['steps']:
        handler = _is_consumption(model, s) or _v8633_is_inbound_web_result(model, s)
        if not handler:
            continue
        for k in s.get('children') or []:
            c = byo[k]
            if c['blocking'] and c['sync'] and _v8636_outbound_external_or_legacy(model, c):
                dep = c.get('target_system') or c.get('system') or 'внешняя система'
                out.append(F('medium', 'Асинхронный обработчик блокируется на внешнем вызове.',
                             f"{s['order']} «{s['name']}» → {c['order']} «{c['name']}» ({dep})",
                             'Потребитель очереди или webhook синхронно ждёт внешнюю систему: её задержка накапливает consumer lag и съедает таймаут доставки, поэтому выигрыш от асинхронности теряется.',
                             'Вынесите внешний вызов в отдельный шаг или очередь; добавьте timeout и circuit breaker; дообработка должна быть идемпотентной, чтобы повторная доставка не дублировала внешний вызов.'))
    return out


def _r_fanin_partial_v8636(model):
    out, byo = [], model['graph']['by_order']
    for s in model['graph']['joins']:
        if not s['blocking']:
            continue
        branches = [byo[d] for d in s['deps'] if d in byo]
        risky = [b for b in branches if _v8636_outbound_external_or_legacy(model, b) or not b['sync']]
        blob = (_low(s['name']) + ' ' + _low(s['data_in']) + ' ' + _low(s['data_out']) + ' ' + _low(s['compensation']))
        policy = bool(re.search(r'partial|части|fallback|деград|best.?effort|optional|тайм.?аут.*ветв', blob))
        if risky and not policy:
            out.append(F('high', 'Агрегация ветвей выполняется без политики частичного отказа.',
                         f"Шаг {s['order']} «{s['name']}» ждёт {len(branches)} ветвей: " + ', '.join((b.get('target_system') or b.get('system')) for b in branches),
                         'Join синхронно ждёт несколько ветвей, среди которых есть внешние или асинхронные зависимости: одна медленная или упавшая ветвь может заблокировать или уронить весь агрегат поскольку латентность определяется самой медленной ветвью.',
                         'Задайте timeout на каждую ветвь и partial response: отдавайте собранные данные и помечайте недостающие блоки; используйте деградацию вместо полного отказа и кэш последних значений ветвей.'))
    return out


def _r_hot_read_v8636(model):
    meta = model['meta']
    if meta['read_freq'] not in ('high', 'very_high'):
        return []
    if not (meta['customer_visible'] or 0 < meta['sla_ms'] <= 1000):
    ...
    for s in model['graph']['critical_path']:
        dep = _v8636_dependency_endpoint(model, s)
        role = _v8636_role(model, dep)
        if (s['blocking'] and s['sync'] and not s['writes_entity']
                and (_v8636_outbound_external_or_legacy(model, s) or role in ('db', 'legacy'))):
            blob = _low(s['name']) + ' ' + _low(s['data_in']) + ' ' + _low(s['data_out'])
            if not re.search(r'кэш|cache|projection|проекци|read.?model|материализ|cdn', blob):
                return [F('medium', 'Горячее чтение идёт в источник без кэша или проекции.',
                          f"Шаг {s['order']} «{s['name']}» → {dep or s['system']}",
                          'Частое чтение синхронно обращается к источнику на критическом пути: его нагрузка и latency становятся вашими, а сам источник превращается в узкое место.',
                          'Добавьте кэш с TTL и инвалидацией или подготовленную проекцию (read-model/CQRS); для статических данных используйте CDN, а в источник обращайтесь только при промахе кэша.')]
    return []


def _r_analytics_core_v8636(model):
    out = []
    for s in model['steps']:
        dep = _v8636_dependency_endpoint(model, s) or s.get('system')
        role = _v8636_role(model, dep)
        looks_dwh = role == 'analytics' or re.search(r'dwh|анали|витрин', _low(dep))
        if looks_dwh and s['blocking']:
            out.append(F('high', 'DWH или аналитика находятся в операционном core-flow.',
                         f"Шаг {s['order']} «{s['name']}» → {dep}",
                         'Аналитическое хранилище обычно не обеспечивает операционный SLA: его деградация не должна останавливать основной бизнес-процесс.',
                         'Сделайте шаг non-blocking: используйте CDC, ETL или событие после фиксации результата в операционной системе.'))
    return out


for _r in RULES:
    if _r.get('id') == 'unstable_dependency':
        _r['fn'] = _r_unstable_v8636
    elif _r.get('id') == 'capacity_vs_limit':
        _r['fn'] = _r_capacity_v8636
    elif _r.get('id') == 'blocking_in_async_handler':
        _r['fn'] = _r_blocking_in_async_v8636
    elif _r.get('id') == 'fanin_partial_failure':
        _r['fn'] = _r_fanin_partial_v8636
    elif _r.get('id') == 'hot_read_no_cache':
        _r['fn'] = _r_hot_read_v8636
    elif _r.get('id') == 'analytics_in_core':
        _r['fn'] = _r_analytics_core_v8636

# ---------------------------------------------------------------------------
# v8.6.37: semantic route external flag fix.
# In the current UI a relation has source_system -> target_system and ``system``
# can be the initiator/executor. For inbound calls like Client -> API, the
# executor may be an external client, but this is not an outbound external
# dependency. Earlier we OR-ed this old executor-based flag, so long purely
# internal sync chains were incorrectly escalated as if they depended on an
# external provider. When source/target route fields are present, external means
# only an outbound call to external/legacy target. Old payloads without route
# fields keep the historical executor-based interpretation.
_PREV_BUILD_GRAPH_V8637 = build_graph

def _v8637_has_semantic_route(step):
    return bool(step.get('source_system') or step.get('target_system'))


def build_graph(model):
    model = _PREV_BUILD_GRAPH_V8637(model)
    for s in model.get('steps', []):
        if _v8637_has_semantic_route(s):
            s['external'] = _v8636_outbound_external_or_legacy(model, s)
        else:
            s['external'] = _v8636_role(model, s.get('system')) in ('external', 'legacy')
    model['graph']['external_blocking'] = [
        s for s in model.get('steps', [])
        if s.get('blocking') and _v8636_outbound_external_or_legacy(model, s)
    ]
    return model

# ---------------------------------------------------------------------------
# v8.6.38: fan-in partial failure должен применяться к бизнес-агрегации, а не к
# сквозным контролям типа Audit Log/журнал аудита. Иначе отчёт выдаёт ложный
# high-блокер на аудиторский шаг, который просто фиксирует результат нескольких ветвей.
_PREV_R_FANIN_PARTIAL_V8638 = _r_fanin_partial_v8636


def _v8638_is_audit_or_control_step(step):
    blob = _low(' '.join([step.get('name') or '', step.get('target_system') or '', step.get('system') or '', step.get('channel') or '']))
    return bool(re.search(r'аудит|audit|журнал аудита|audit log|evidence|логирован|observability|наблюдаем', blob))


def _r_fanin_partial_v8638(model):
    original_joins = list(model.get('graph', {}).get('joins') or [])
    model['graph']['joins'] = [s for s in original_joins if not _v8638_is_audit_or_control_step(s)]
    try:
        return _PREV_R_FANIN_PARTIAL_V8638(model)
    finally:
        model['graph']['joins'] = original_joins


_r_fanin_partial_v8636 = _r_fanin_partial_v8638
for _r in RULES:
    if _r.get('id') == 'fanin_partial_failure':
        _r['fn'] = _r_fanin_partial_v8638

# v8.6.38b: улучшение fan-in policy detection.
# Старое правило считало само слово "partial" доказательством политики, даже если
# пользователь написал "без partial response policy" / "without partial response".
# Это скрывало реальный риск join'а нескольких ветвей.

def _v8638_has_partial_policy(text):
    blob = _low(text)
    if re.search(r'(без|нет|отсутств|no|without)\s+(partial|частич|fallback|деград|best.?effort|тайм.?аут.*ветв|политик)', blob):
        return False
    return bool(re.search(r'partial|части|fallback|деград|best.?effort|optional|тайм.?аут.*ветв', blob))


def _r_fanin_partial_v8638b(model):
    out, byo = [], model['graph']['by_order']
    for s in model['graph']['joins']:
        if not s['blocking'] or _v8638_is_audit_or_control_step(s):
            continue
        branches = [byo[d] for d in s['deps'] if d in byo]
        risky = [b for b in branches if _v8636_outbound_external_or_legacy(model, b) or not b['sync']]
        blob = (_low(s['name']) + ' ' + _low(s['data_in']) + ' ' + _low(s['data_out']) + ' ' + _low(s['compensation']))
        if risky and not _v8638_has_partial_policy(blob):
            out.append(F('high', 'Агрегация ветвей выполняется без политики частичного отказа.',
                         f"Шаг {s['order']} «{s['name']}» ждёт {len(branches)} ветвей: " + ', '.join((b.get('target_system') or b.get('system')) for b in branches),
                         'Join синхронно ждёт несколько ветвей, среди которых есть внешние или асинхронные зависимости: одна медленная или упавшая ветвь может заблокировать или уронить весь агрегат поскольку латентность определяется самой медленной ветвью.',
                         'Задайте timeout на каждую ветвь и partial response: отдавайте собранные данные и помечайте недостающие блоки; используйте деградацию вместо полного отказа и кэш последних значений ветвей.'))
...
_r_fanin_partial_v8636 = _r_fanin_partial_v8638b
for _r in RULES:
    if _r.get('id') == 'fanin_partial_failure':
        _r['fn'] = _r_fanin_partial_v8638b

# ---------------------------------------------------------------------------
# v8.6.41: строгая валидация новых связей и каналов.
# На реальном ошибочном прогоне обнаружилось, что неизвестный channel молча
# превращался в REST, а неизвестный target_system пропускался. Для UI это плохо:
# пользователь получает красивый отчёт по схеме, которой фактически не существует.
_PREV_NORMALIZE_V8641 = normalize

def normalize(payload):
    model = _PREV_NORMALIZE_V8641(payload)
    raw_steps = list((payload or {}).get('steps') or [])
    # Привязываем исходные поля к нормализованным шагам по позиции после сортировки
    # order. Это достаточно стабильно для проверки channel/source/target и не влияет
    # на старую совместимость, где source/target могли не заполняться.
    raw_sorted = sorted(enumerate(raw_steps), key=lambda it: (_int((it[1] or {}).get('order'), it[0] + 1), it[0]))
    for st, (_, raw) in zip(model.get('steps') or [], raw_sorted):
        raw = raw or {}
        st['_input_channel'] = _low(raw.get('channel'))
        st['_input_source_system'] = _s(raw.get('source_system'))
        st['_input_target_system'] = _s(raw.get('target_system'))
        st['_input_executor_system'] = _s(raw.get('system'))
    return model

_PREV_VALIDATE_V8641 = validate

def validate(model):
    errors = list(_PREV_VALIDATE_V8641(model))
    systems = model.get('systems') or {}
    for s in model.get('steps') or []:
        raw_ch = s.get('_input_channel') or ''
        if raw_ch and raw_ch not in ALL_CHANNELS:
            errors.append(f"Шаг «{s['name']}»: неизвестный канал взаимодействия «{raw_ch}».")
        for field, label in (('_input_source_system', 'источник'), ('_input_target_system', 'получатель')):
            val = s.get(field) or ''
            if val and val not in systems:
                errors.append(f"Шаг «{s['name']}»: неизвестный {label} связи «{val}». Добавьте участника процесса или исправьте связь.")
    return errors

# v8.6.41b: fan-in без политики частичного отказа опасен не только в
# синхронной цепочке. Даже асинхронная бизнес-агрегация должна иметь правила:
# сколько ждать ветви, что делать с частичным результатом, где ручной разбор.
def _r_fanin_partial_v8641(model):
    out, byo = [], model['graph']['by_order']
    for s in model['graph']['joins']:
        if _v8638_is_audit_or_control_step(s):
            continue
        branches = [byo[d] for d in s['deps'] if d in byo]
        risky = [b for b in branches if _v8636_outbound_external_or_legacy(model, b) or not b['sync']]
        blob = (_low(s.get('name')) + ' ' + _low(s.get('data_in')) + ' ' + _low(s.get('data_out')) + ' ' + _low(s.get('compensation')))
        if risky and not _v8638_has_partial_policy(blob):
            sev = 'high' if s.get('blocking') else 'medium'
            out.append(F(sev, 'Агрегация ветвей выполняется без политики частичного отказа.',
                         f"Шаг {s['order']} «{s['name']}» ждёт {len(branches)} ветвей: " + ', '.join((b.get('target_system') or b.get('system')) for b in branches),
                         'Сведение нескольких ветвей зависит от внешних, асинхронных или потенциально медленных источников. Без политики ожидания одна ветвь может задержать результат, скрыть частичный отказ или оставить процесс в неопределённом статусе.',
                         'Задайте timeout на каждую ветвь, правило частичного результата, fallback/деградацию и ручной разбор для незавершённых ветвей.'))
    return out

_r_fanin_partial_v8636 = _r_fanin_partial_v8641
for _r in RULES:
    if _r.get('id') == 'fanin_partial_failure':
        _r['fn'] = _r_fanin_partial_v8641

# v8.6.41c: source_system может быть внешним актором/устройством в старых
# payload без явного объявления в participants. Не блокируем отчёт только из-за
# неизвестного источника; строгой остаётся проверка получателя и канала.
_PREV_VALIDATE_V8641C = validate

def validate(model):
    errors = list(_PREV_VALIDATE_V8641C(model))
    return [e for e in errors if 'неизвестный источник связи' not in e]

# v8.6.41d: неизвестный target_system лучше показывать в отчёте как
# блокер схемы, а не останавливать analyze. Так пользователь получает разбор
# ошибочного процесса и видит, что именно надо исправить. Unknown channel остаётся
# фатальной ошибкой: его нельзя безопасно подменять на REST.
_PREV_VALIDATE_V8641D = validate

def validate(model):
    errors = list(_PREV_VALIDATE_V8641D(model))
    return [e for e in errors if 'неизвестный источник связи' not in e and 'неизвестный получатель связи' not in e]

@rule('undeclared_route_endpoint', 'Схема процесса')
def r_undeclared_route_endpoint_v8641(model):
    out = []
    systems = model.get('systems') or {}
    for s in model.get('steps') or []:
        target = s.get('_input_target_system') or s.get('target_system') or ''
        if target and target not in systems:
            out.append(F('critical', 'Получатель связи не объявлен среди участников процесса.',
                         f"Шаг {s['order']} «{s['name']}» → {target}",
                         'Ядро не может надёжно определить роль, критичность, владельца и ограничения получателя, поэтому отчёт по такой связи нельзя считать готовым к постановке.',
                         'Добавьте получателя в список участников процесса с ролью и критичностью либо исправьте target_system в связи.'))
    return out

# ---------------------------------------------------------------------------
# v8.6.42: production calibration of noisy rules.
# Full production-style runs showed that the core was stable but several rules
# were too literal for semantic routes: infra/control edges were treated as
# business external dependencies, analytics-internal steps were treated as
# operational DWH calls, outbox/CDC controls written in natural text were not
# recognised, and scoring collapsed useful drafts to 0/10 too often.

_INFRA_CONTROL_CHANNELS_V8642 = {
    'cdn', 'auth_oidc', 'vault', 'observability', 'service_mesh', 'api_gateway'
}
_ANALYTICS_CHANNELS_V8642 = {
    'cdc', 'etl', 'batch', 'airflow', 'spark', 'dbt', 'clickhouse',
    'data_warehouse', 'data_lake', 'lakehouse'
}
_STORAGE_CHANNELS_V8642 = {
    'db', 'read_replica', 'db_sharding', 'mongodb', 'cassandra', 'dynamodb',
    'clickhouse', 'data_warehouse', 'data_lake', 'lakehouse', 'object_storage',
    'file'
}
_REAL_SYNC_BUSINESS_CHANNELS_V8642 = {
    'rest', 'graphql', 'odata', 'grpc', 'soap', 'db', 'read_replica',
    'db_sharding', 'mongodb', 'dynamodb', 'search', 'vector_db'
}


def _v8642_blob(*parts):
    return _low(' '.join(str(p or '') for p in parts))


def _v8642_route_role(model, name):
    return ((model.get('systems') or {}).get(name) or {}).get('role', '')


def _v8642_is_control_or_infra_edge(step):
    ch = step.get('channel')
    blob = _v8642_blob(step.get('name'), step.get('system'), step.get('target_system'), step.get('data_in'), step.get('data_out'))
    if ch in _INFRA_CONTROL_CHANNELS_V8642:
        return True
    return bool(re.search(r'vault|kms|secret|секрет|ключ|oidc|oauth|auth|авторизац|cdn|статик|observability|metric|trace|лог|наблюдаем', blob))


def _v8642_is_outbound_external_or_legacy(model, step):
    """External dependency that really belongs to the business flow.

    Excludes edges that are merely CDN delivery, auth, gateway/mesh plumbing,
    vault/secrets or observability. Those controls may still appear in the
    report, but they must not make the business architecture red by themselves.
    """
    src = step.get('source_system') or ''
    tgt = step.get('target_system') or ''
    sysname = step.get('system') or ''
    src_role = _v8642_route_role(model, src)
    tgt_role = _v8642_route_role(model, tgt)
    sys_role = _v8642_route_role(model, sysname)
    if _v8642_is_control_or_infra_edge(step):
        return False
    if tgt and tgt_role in ('external', 'legacy') and src_role not in ('external', 'legacy'):
        return True
    if sys_role in ('external', 'legacy') and src_role not in ('external', 'legacy') and (not tgt or tgt == sysname):
...
# Replace the helper used by semantic route rules.
_v8636_outbound_external_or_legacy = _v8642_is_outbound_external_or_legacy

_PREV_BUILD_GRAPH_V8642 = build_graph

def build_graph(model):
    model = _PREV_BUILD_GRAPH_V8642(model)
    for s in model.get('steps', []):
        if _v8637_has_semantic_route(s):
            s['external'] = _v8642_is_outbound_external_or_legacy(model, s)
        else:
            s['external'] = _v8642_route_role(model, s.get('system')) in ('external', 'legacy') and not _v8642_is_control_or_infra_edge(s)
    model['graph']['external_blocking'] = [
        s for s in model.get('steps', [])
        if s.get('blocking') and _v8642_is_outbound_external_or_legacy(model, s)
    ]

    # Recalculate the displayed sync-chain depth without pure infra/control hops.
    # This keeps true long REST/SOAP/DB chains visible, but stops capability maps
    # like CDN -> Auth -> Gateway -> Mesh -> Cache -> DB from becoming 0/10 only
    # because every supporting component was entered as a separate step.
    by_order = model['graph']['by_order']
    def counts_for_sync_chain(st):
        return (st.get('sync') and st.get('blocking')
                and st.get('channel') in _REAL_SYNC_BUSINESS_CHANNELS_V8642
                and not _v8642_is_control_or_infra_edge(st))
    srun, sparent = {}, {}
    def run(order, stack):
        if order in srun:
            return srun[order]
        if order in stack or order not in by_order:
            return 0
        st = by_order[order]
        if not counts_for_sync_chain(st):
            best, best_p = 0, None
            for d in st.get('deps') or []:
                r = run(d, stack | {order})
                if r > best:
                    best, best_p = r, d
            srun[order], sparent[order] = best, best_p
            return best
        best, best_p = 0, None
        for d in st.get('deps') or []:
            p = by_order.get(d)
            if p and p.get('blocking'):
                r = run(d, stack | {order})
                if r > best:
                    best, best_p = r, d
        srun[order], sparent[order] = 1 + best, best_p
        return srun[order]
    for st in model.get('steps', []):
        run(st['order'], frozenset())
    end = max(srun, key=lambda o: (srun[o], o), default=None)
    depth = srun.get(end, 0) if end is not None else 0
    path, cur, guard = [], end, set()
    while depth and cur is not None and cur not in guard:
        guard.add(cur)
        st = by_order.get(cur)
        if st and counts_for_sync_chain(st):
            path.append(st)
        cur = sparent.get(cur)
    path.reverse()
    model['graph']['sync_depth'] = len(path)
    model['graph']['sync_chain_path'] = path
    return model


def _v8642_step_has_async_ancestor(model, step):
    by_order = model.get('graph', {}).get('by_order') or {}
    seen = set()
    stack = list(step.get('deps') or [])
    while stack:
        o = stack.pop()
        if o in seen:
            continue
        ...
        if not p:
            continue
        if not p.get('sync') or not p.get('blocking') or _is_consumption(model, p) or p.get('channel') in BROKER_CHANNELS:
            return True
        stack.extend(p.get('deps') or [])
    return False


def _r_external_blocking_v8642(model):
    out = []
    meta = model['meta']
    critical_orders = {s.get('order') for s in (model.get('graph') or {}).get('critical_path') or []}
    for s in model['graph']['external_blocking']:
        target = s.get('target_system') or s.get('system') or 'внешняя система'
        # A blocking call from an async worker is important, but it does not block
        # the original user's HTTP response. Keep it high, not critical.
        if s.get('order') in critical_orders and not _v8642_step_has_async_ancestor(model, s) and meta['customer_visible'] and 0 < meta['sla_ms'] <= 1000:
            sev = 'critical'
        else:
            sev = 'high'
        out.append(F(sev, 'Процесс блокируется на вызове внешней системы.',
                     f"Шаг {s['order']} «{s['name']}» → {target}",
                     'Внешняя система находится вне вашего контроля: её деградация напрямую ухудшает ваш SLA или копит очередь обработки.',
                     'Настройте таймаут, предохранитель внешнего вызова, лимит запросов и запасной сценарий; если шаг находится в пользовательском пути, рассмотрите асинхронную обработку с trackingId.'))
    return out


def _r_analytics_core_v8642(model):
    out = []
    for s in model['steps']:
        dep = _v8636_dependency_endpoint(model, s) or s.get('system')
        role = _v8642_route_role(model, dep)
        looks_dwh = role == 'analytics' or re.search(r'dwh|анали|витрин|warehouse|lakehouse|data lake|clickhouse', _low(dep))
        if not (looks_dwh and s.get('blocking')):
            continue
        src_role = _v8642_route_role(model, s.get('source_system'))
        sys_role = _v8642_route_role(model, s.get('system'))
        # Analytics-internal modelling/loading steps are allowed even if the UI
        # marked them as blocking, because they are not the operational user flow.
        if s.get('channel') in _ANALYTICS_CHANNELS_V8642 and src_role == 'analytics' and sys_role == 'analytics':
            continue
        if _v8642_is_control_or_infra_edge(s):
            continue
        out.append(F('high', 'DWH или аналитика находятся в операционном core-flow.',
                     f"Шаг {s['order']} «{s['name']}» → {dep}",
                     'Аналитическое хранилище обычно не обеспечивает операционный SLA: его деградация не должна останавливать основной бизнес-процесс.',
...
def _v8642_has_outbox_marker(step):
    return bool(re.search(r'outbox|исходящ', _v8642_blob(step.get('name'), step.get('source_system'), step.get('target_system'), step.get('system'), step.get('compensation'), step.get('data_in'), step.get('data_out'), step.get('failure_policy'))))


def _r_dual_write_v8642(model):
    by_system = {}
    systems = model.get('systems') or {}
    for s in model['steps']:
        by_system.setdefault(s['system'], []).append(s)
    out = []
    for sysname, ss in by_system.items():
        if _v8642_route_role(model, sysname) in ('external', 'legacy'):
            continue
        writes = any(x.get('writes_entity') or _is_actual_entity_write(x, systems) for x in ss)
        publishes = [x for x in ss if x['channel'] in BROKER_CHANNELS and not _is_consumption(model, x)]
        mentions_outbox = any(_v8642_has_outbox_marker(x) for x in ss)
        if writes and publishes and not mentions_outbox:
            places = ', '.join(f"шаг {x['order']} «{x['name']}»" for x in publishes)
            out.append(F('high', 'Система одновременно пишет в БД и публикует событие без таблицы исходящих сообщений.',
                         f"{sysname}: {places}",
...
def _r_cdc_projection_v8642(model):
    out = []
    for s in model['steps']:
        if s['channel'] != 'cdc':
            continue
        blob = _v8642_blob(s.get('name'), s.get('data_in'), s.get('data_out'), s.get('compensation'), s.get('failure_policy'))
        has_position = bool(re.search(r'lsn|watermark|offset|позици|контрольн.*отмет', blob))
        has_reconcile = bool(re.search(r'reconciliation|сверк|record.?count|checksum|полнот', blob))
        has_replay = bool(re.search(r'replay|backfill|дозагруз|повторн|resync', blob))
        if has_position and has_reconcile and has_replay:
            continue
        out.append(F('medium', 'Для CDC-потока не описаны обязательные контроли.',
                     f"Шаг {s['order']} «{s['name']}» ({s.get('system')})",
                     'CDC может незаметно сломаться на пропусках позиций, удалениях и эволюции схемы источника, из-за чего проекция тихо расходится с источником истины.',
                     'Используйте LSN или watermark с контролем пропусков; добавьте обработку delete-событий, политику эволюции схемы, идемпотентную проекцию, регулярную reconciliation-сверку и replay за период.'))
    return out


def _r_poison_retry_v8642(model):
    out = []
    for s in model['steps']:
        blob = _v8642_blob(s.get('name'), s.get('compensation'), s.get('data_in'), s.get('data_out'), s.get('failure_policy'))
        has_dlq = bool(re.search(r'dlq|dead.?letter|карантин|лимит|max|попыт|backoff|ручн.*разбор|parking', blob))
        if s['retry'] == 'auto' and not s['sync'] and not has_dlq:
            out.append(F('medium', 'Retry настроен без лимита попыток и DLQ.',
                         f"Шаг {s['order']} «{s['name']}»",
                         '«Ядовитое» сообщение будет снова и снова возвращаться в очередь: это создаст бесконечный цикл ошибок, лишнюю нагрузку на CPU и переполнение очереди.',
                         'Добавьте счётчик попыток и экспоненциальный backoff; после заданного числа попыток отправляйте сообщение в DLQ или карантин с алертом и процедурой replay.'))
    return out


def _r_money_v8642(model):
    meta, steps = model['meta'], model['steps']
    if meta['money'] != 'direct':
        return []
    out = []
    writers = model['graph']['writers']
    if len(writers) > 1:
        write_steps = [s for s in steps if s.get('writes_entity')]
        blob = _v8642_blob(*(x.get('name') + ' ' + x.get('data_in') + ' ' + x.get('data_out') + ' ' + x.get('compensation') for x in write_steps))
        controlled = bool(re.search(r'idempotency|идемпотент|status.?version|version|optimistic|ledger|проводк|уникальн|lock|блокиров', blob))
        sev = 'high' if controlled and all(_v8642_route_role(model, w) not in ('external', 'legacy') for w in writers) else 'critical'
        out.append(F(sev, 'Финансовую сущность изменяют несколько систем одновременно.',
                     ', '.join(writers),
                     'Несколько писателей финансового состояния требуют явной модели владения, версионности и идемпотентности; без этого возможны расхождения и двойное применение операции.',
                     'Назначьте владельца агрегата или ledger; остальные системы должны отправлять команды/события. Если несколько внутренних обработчиков всё же пишут статус, зафиксируйте optimistic lock, statusVersion, idempotencyKey и правила конфликтов.'))
    if not any(s['idempotency'] != 'none' for s in steps if s['writes_entity']):
        out.append(F('critical', 'Финансовая запись выполняется без защиты от повторного применения.',
                     'Шаги, пишущие сущность',
                     'При сетевых повторах одна и та же бизнес-операция может быть применена дважды.',
...
def score(findings, finding_groups=None):
    """Production-calibrated score.

    Structural criticals still make the verdict red, but the numeric value no
    longer collapses useful complex drafts to 0/10 merely because several
    improvement rules fired. This makes the score actionable: 0-2 is broken
    schema/design, 3-5 is risky draft, 6-7 is workable with controls, 8+ is good.
    """
    groups = finding_groups or group_findings(findings)
    base_penalty = {'critical': 2.0, 'high': 0.75, 'medium': 0.28, 'info': 0.0}
    extra_step = {'critical': 0.08, 'high': 0.04, 'medium': 0.02, 'info': 0.0}
    extra_cap = {'critical': 0.45, 'high': 0.25, 'medium': 0.12, 'info': 0.0}
    total = 0.0
    for g in groups:
        sev = g.get('severity') or 'info'
        total += base_penalty.get(sev, 0.0)
        total += min(extra_cap.get(sev, 0.0), max(0, int(g.get('count') or 1) - 1) * extra_step.get(sev, 0.0))
    val = max(0.0, round(10.0 - total, 1))
    crit = sum(1 for f in findings if f['severity'] == 'critical')
    critical_groups = sum(1 for g in groups if g.get('severity') == 'critical')
    schema_critical = any((g.get('severity') == 'critical' and str(g.get('rule') or '').startswith(('invalid_', 'undeclared_', 'schema')))
                          for g in groups)
    if schema_critical:
        verdict, color = 'НЕ ГОТОВО: исправьте схему процесса', 'red'
    elif crit:
        verdict, color = 'НЕ ГОТОВО: есть блокирующие риски', 'red'
    elif val >= 8:
        verdict, color = 'ГОТОВО к проектированию с контролями', 'green'
        ...
        verdict, color = 'НЕ ГОТОВО: слишком много рисков', 'red'
    return {'score': val, 'verdict': verdict, 'color': color,
            'counts': {k: sum(1 for f in findings if f['severity'] == k) for k in ('critical', 'high', 'medium', 'info')},
            'group_counts': {k: sum(1 for g in groups if g.get('severity') == k) for k in ('critical', 'high', 'medium', 'info')},
            'critical_groups': critical_groups}

for _r in RULES:
    if _r.get('id') == 'external_blocking':
        _r['fn'] = _r_external_blocking_v8642
    elif _r.get('id') == 'analytics_in_core':
        _r['fn'] = _r_analytics_core_v8642
    elif _r.get('id') == 'dual_write':
        _r['fn'] = _r_dual_write_v8642
    elif _r.get('id') == 'cdc_projection_controls':
        _r['fn'] = _r_cdc_projection_v8642
    elif _r.get('id') == 'poison_retry':
        _r['fn'] = _r_poison_retry_v8642
    elif _r.get('id') == 'money_controls':
        _r['fn'] = _r_money_v8642

# v8.6.42b: final calibration after production audit.
def _r_sync_chain_v8642(model):
    g, meta = model['graph'], model['meta']
    if g['sync_depth'] < 3:
        return []
    names = ' → '.join(s['name'] for s in g['sync_chain_path'])
    # 3-4 meaningful blocking hops are a high risk, but not an automatic
    # production blocker; 5+ hops in a customer-visible fast path is critical.
    sev = 'critical' if (g['sync_depth'] >= 5 and meta['customer_visible'] and any(s['external'] for s in g['sync_chain_path'])) else 'high'
    return [F(sev, f"В процессе есть слишком длинная синхронная цепочка: {g['sync_depth']} блокирующих шага подряд.",
              names,
              'Каждое синхронное звено увеличивает вероятность отказа и добавляет задержку к общему времени ответа; если откажет любое звено, весь пользовательский или системный запрос завершится ошибкой.',
              'Разорвите цепочку: после первого подтверждённого шага переводите дальнейшую обработку в события или очередь, а клиенту возвращайте trackingId и понятную статусную модель процесса.')]


def _r_poison_retry_v8642b(model):
    out = []
    for s in model['steps']:
        if _v8642_is_control_or_infra_edge(s):
            continue
        blob = _v8642_blob(s.get('name'), s.get('compensation'), s.get('data_in'), s.get('data_out'), s.get('failure_policy'))
        has_dlq = bool(re.search(r'dlq|dead.?letter|карантин|quarantine|лимит|max|попыт|backoff|ручн.*разбор|parking|best.?effort', blob))
        if s['retry'] == 'auto' and not s['sync'] and not has_dlq:
            out.append(F('medium', 'Retry настроен без лимита попыток и DLQ.',
                         f"Шаг {s['order']} «{s['name']}»",
                         '«Ядовитое» сообщение будет снова и снова возвращаться в очередь: это создаст бесконечный цикл ошибок, лишнюю нагрузку на CPU и переполнение очереди.',
...
def score(findings, finding_groups=None):
    groups = finding_groups or group_findings(findings)
    base_penalty = {'critical': 2.0, 'high': 0.58, 'medium': 0.18, 'info': 0.0}
    extra_step = {'critical': 0.06, 'high': 0.03, 'medium': 0.015, 'info': 0.0}
    extra_cap = {'critical': 0.35, 'high': 0.18, 'medium': 0.08, 'info': 0.0}
    total = 0.0
    for g in groups:
        sev = g.get('severity') or 'info'
        total += base_penalty.get(sev, 0.0)
...
for _r in RULES:
    if _r.get('id') == 'sync_chain_depth':
        _r['fn'] = _r_sync_chain_v8642
    elif _r.get('id') == 'poison_retry':
        _r['fn'] = _r_poison_retry_v8642b

# v8.6.42c: a design with high-severity findings must not be marked green.
# Green now means: no critical and no high groups; yellow means usable draft with
# high risks that still require controls before production.
def score(findings, finding_groups=None):
    groups = finding_groups or group_findings(findings)
    base_penalty = {'critical': 2.0, 'high': 0.58, 'medium': 0.18, 'info': 0.0}
    extra_step = {'critical': 0.06, 'high': 0.03, 'medium': 0.015, 'info': 0.0}
    ...
    if schema_critical:
        ...
        verdict, color = ('УСЛОВНО ГОТОВО: закрыть высокие риски', 'yellow') if val >= 5.5 else ('НЕ ГОТОВО: слишком много рисков', 'red')
    elif val >= 8:
        verdict, color = 'ГОТОВО к проектированию с контролями', 'green'
    elif val >= 5.5:
        verdict, color = 'УСЛОВНО ГОТОВО: закрыть средние риски', 'yellow'
    else:
        verdict, color = 'НЕ ГОТОВО: слишком много рисков', 'red'
    return {'score': val, 'verdict': verdict, 'color': color,
            'counts': {k: sum(1 for f in findings if f['severity'] == k) for k in ('critical', 'high', 'medium', 'info')},
...
# ---------------------------------------------------------------------------
# v8.6.56: жёсткая production-доводка после пользовательского аудита.
# 1) Неизвестный получатель больше не превращается в «низкий отчёт»: схема
#    блокируется до формирования отчёта, потому что связь указывает на узел,
#    которого нет среди участников процесса.
# 2) Набор из нескольких high-risk production-пробелов больше не получает
#    мягкий yellow 6+. Если одновременно отсутствуют внешние контроли,
#    восстановление, таймауты/контракты и наблюдаемость, это красный черновик.
_PREV_VALIDATE_V8655 = validate

def validate(model):
    errors = list(_PREV_VALIDATE_V8655(model))
    systems = model.get('systems') or {}
    seen = set(errors)
    for s in model.get('steps') or []:
        target = (s.get('_input_target_system') or s.get('target_system') or '').strip()
        if target and target not in systems:
            msg = (f"Шаг «{s.get('name') or s.get('order')}»: неизвестный получатель связи «{target}». "
                   "Добавьте получателя в список участников процесса или исправьте связь перед формированием отчёта.")
            if msg not in seen:
                errors.append(msg)
                seen.add(msg)
    return errors

_PREV_SCORE_V8655 = score

def score(findings, finding_groups=None):
    verdict = dict(_PREV_SCORE_V8655(findings, finding_groups))
    groups = finding_groups or group_findings(findings)
    fired = {str(g.get('rule') or '') for g in groups}
    high_groups = sum(1 for g in groups if g.get('severity') == 'high')
    medium_groups = sum(1 for g in groups if g.get('severity') == 'medium')
    has_structural_schema_blocker = any(
        g.get('severity') == 'critical' and str(g.get('rule') or '').startswith(('invalid_', 'undeclared_', 'schema'))
        for g in groups
    )
    dangerous_control_bundle = (
        {'external_blocking', 'async_without_recovery'} <= fired
        and ({'missing_timeouts', 'event_core_fields', 'no_correlation_id', 'observability_missing'} & fired)
    )
    many_high_risks = high_groups >= 4 and medium_groups >= 3
    if not has_structural_schema_blocker and verdict.get('color') != 'red' and (dangerous_control_bundle or many_high_risks):
        verdict['score'] = min(float(verdict.get('score') or 0), 5.0)
        verdict['score'] = round(verdict['score'], 1)
        verdict['verdict'] = 'НЕ ГОТОВО: слишком много production-рисков и отсутствуют базовые контроли'
        verdict['color'] = 'red'
    return verdict

# ---------------------------------------------------------------------------
# v8.6.59: production-readiness calibration.
# Цель правки: отличать реально опасную схему без контролей от сложной,
# но управляемой production-архитектуры. Раньше валидные банковские/e-commerce
# кейсы могли получать 0-1/10 из-за нескольких критичных штрафов, хотя в них
# уже были таймауты, circuit breaker, fallback, outbox, DLQ и ручной разбор.

def _v8659_blob(*parts):
    return _low(' '.join(str(x or '') for x in parts))


def _v8659_has_controls(step, *families):
    blob = _v8659_blob(step.get('name'), step.get('data_in'), step.get('data_out'), step.get('compensation'), step.get('failure_policy'))
    checks = {
        'timeout': r'timeout|тайм.?аут|deadline|budget',
        'breaker': r'circuit|breaker|предохран',
        'fallback': r'fallback|деград|manual|ручн|partial|отлож|tracking',
        'retry_safe': r'dlq|dead.?letter|карантин|backoff|jitter|лимит|max|retry window|replay',
        'idempotency': r'idempot|идемпотент|unique|уникальн|dedup|дедуп|inbox|operationid|eventid',
        'rate': r'rate.?limit|лимит запрос|backoff|jitter|queue|очеред|буфер|throttl|bulkhead|деград|кэш|cache',
        'ledger': r'ledger|проводк|status.?version|optimistic|lock|блокиров|single writer|един.*писат|owner|владел',
    }
    return all(re.search(checks.get(f, f), blob) for f in families)


def _r_capacity_v8659(model):
    meta = model['meta']
    out = []
    if not meta.get('peak_rps'):
        return out
    hit = set()
    for s in model['steps']:
        dep = _v8636_dependency_endpoint(model, s)
        if not dep or dep not in model['systems'] or dep in hit:
            continue
        limit = model['systems'][dep].get('rate_limit_rps', 0)
        if not limit or meta['peak_rps'] <= limit:
            continue
        hit.add(dep)
        ratio = float(meta['peak_rps']) / max(1.0, float(limit))
        controlled = _v8659_has_controls(s, 'rate') or _v8659_has_controls(s, 'fallback')
        sev = 'critical' if ratio >= 3.0 and not controlled else 'high'
        out.append(F(sev, 'Пиковая нагрузка превышает rate limit зависимости',
                     f"{dep}: пик {meta['peak_rps']} RPS > лимит {limit} RPS",
                     'В пик каждый запрос сверх лимита может получить 429/отказ; без выравнивания нагрузки это превращается в шторм ошибок.',
                     'Добавьте очередь или буфер перед зависимостью, клиентский rate limiter, backoff с jitter, деградацию или кэш; отдельно согласуйте повышение лимита с владельцем зависимости.'))
    return out


def _r_external_blocking_v8659(model):
    out = []
    meta = model['meta']
    critical_orders = {s.get('order') for s in (model.get('graph') or {}).get('critical_path') or []}
    for s in model['graph']['external_blocking']:
        target = s.get('target_system') or s.get('system') or 'внешняя система'
        controlled = _v8659_has_controls(s, 'timeout') and (_v8659_has_controls(s, 'breaker') or _v8659_has_controls(s, 'fallback'))
        in_user_critical_path = s.get('order') in critical_orders and not _v8642_step_has_async_ancestor(model, s) and meta['customer_visible']
        sev = 'critical' if in_user_critical_path and 0 < meta.get('sla_ms', 0) <= 700 and not controlled else 'high'
        out.append(F(sev, 'Процесс блокируется на вызове внешней системы.',
                     f"Шаг {s['order']} «{s['name']}» → {target}",
                     'Внешняя система находится вне вашего контроля: её деградация напрямую влияет на пользовательский путь или копит очередь обработки.',
                     'Зафиксируйте timeout, circuit breaker, лимит запросов и запасной сценарий. Если результат не нужен сразу, переводите продолжение в асинхронный процесс с trackingId и статусной моделью.'))
    return out


def _r_sla_budget_v8659(model):
    meta, g = model['meta'], model['graph']
    if not meta.get('sla_ms') or not g.get('critical_path'):
        return []
    budget = g.get('critical_budget_ms') or 0
    if budget <= meta['sla_ms']:
        return []
    chain_txt = ' + '.join(f"{s['timeout_ms']}мс ({s['name']})" for s in g['critical_path'] if s.get('timeout_ms')) or 'критический путь'
    ratio = float(budget) / max(1.0, float(meta['sla_ms']))
    blob = _v8659_blob(*(s.get('compensation') for s in g.get('critical_path') or []), meta.get('statuses'), meta.get('lookup_keys'))
    controlled = bool(re.search(r'tracking|status|статус|partial|manual|ручн|fallback|деград|async|асинх', blob))
    sev = 'critical' if ratio >= 2.0 and not controlled else 'high'
    return [F(sev, 'Бюджет таймаутов превышает заявленный SLA.',
              chain_txt,
              f"Сумма таймаутов критического пути {budget} мс > SLA {meta['sla_ms']} мс. Это не всегда ошибка схемы, но без явной деградации пользователь может ждать дольше заявленного срока.",
              'Распределите бюджет времени сверху вниз, распараллельте независимые проверки или возвращайте trackingId с последующей асинхронной обработкой и понятными статусами процесса.')]


def _r_money_v8659(model):
    meta, steps = model['meta'], model['steps']
    if meta.get('money') != 'direct':
        return []
    out = []
    writers = model['graph']['writers']
    if len(writers) > 1:
        write_steps = [s for s in steps if s.get('writes_entity')]
        blob = _v8659_blob(*(x.get('name') + ' ' + x.get('data_in') + ' ' + x.get('data_out') + ' ' + x.get('compensation') for x in write_steps))
        controlled = bool(re.search(r'idempotency|идемпотент|status.?version|version|optimistic|ledger|проводк|уникальн|lock|блокиров|transaction|audit|outbox|saga|manual', blob))
        external_writer = any(_v8642_route_role(model, w) in ('external', 'legacy') for w in writers)
        sev = 'critical' if external_writer or not controlled else 'high'
        out.append(F(sev, 'Финансовую сущность изменяют несколько систем одновременно.',
                     ', '.join(writers),
                     'Несколько писателей финансового состояния допустимы только при явной модели владения, версионности и идемпотентности; иначе возможны расхождения и двойное применение операции.',
                     'Назначьте владельца агрегата или ledger. Если несколько внутренних обработчиков всё же меняют статус, зафиксируйте optimistic lock, statusVersion, idempotencyKey, журнал аудита и правила конфликтов.'))
    if not any(s['idempotency'] != 'none' for s in steps if s.get('writes_entity')):
        out.append(F('critical', 'Финансовая запись выполняется без защиты от повторного применения.',
                     'Шаги, пишущие сущность',
                     'При сетевых повторах одна и та же бизнес-операция может быть применена дважды.',
                     'Используйте operationId с уникальным индексом и ledger с проводками вместо прямого update баланса.'))
...
for _r in RULES:
    if _r.get('id') == 'capacity_vs_limit':
        _r['fn'] = _r_capacity_v8659
    elif _r.get('id') == 'external_blocking':
        _r['fn'] = _r_external_blocking_v8659
    elif _r.get('id') == 'sla_budget':
        _r['fn'] = _r_sla_budget_v8659
    elif _r.get('id') == 'money_controls':
        _r['fn'] = _r_money_v8659

_PREV_SCORE_V8659 = score

def score(findings, finding_groups=None):
    verdict = dict(_PREV_SCORE_V8659(findings, finding_groups))
    groups = finding_groups or group_findings(findings)
    critical_groups = sum(1 for g in groups if g.get('severity') == 'critical')
    high_groups = sum(1 for g in groups if g.get('severity') == 'high')
    medium_groups = sum(1 for g in groups if g.get('severity') == 'medium')
    schema_critical = any((g.get('severity') == 'critical' and str(g.get('rule') or '').startswith(('invalid_', 'undeclared_', 'schema'))) for g in groups)
    if schema_critical:
        return verdict
    # Одна-две блокирующие production-проблемы не должны обнулять полезный сложный черновик:
    # это red, но с оценкой, по которой видно, что схема близка к исправлению.
    if verdict.get('color') == 'red' and critical_groups <= 1 and high_groups <= 5:
        suggested = 5.2 if medium_groups <= 5 else 4.8
        verdict['score'] = max(float(verdict.get('score') or 0), suggested)
        verdict['score'] = round(min(verdict['score'], 5.4), 1)
        verdict['verdict'] = 'НЕ ГОТОВО: остались блокирующие production-риски, но схема исправима'
    return verdict

# v8.6.59b: final score floor for realistic, non-schema production drafts.
_PREV_SCORE_V8659B = score

def score(findings, finding_groups=None):
    verdict = dict(_PREV_SCORE_V8659B(findings, finding_groups))
    groups = finding_groups or group_findings(findings)
    schema_critical = any((g.get('severity') == 'critical' and str(g.get('rule') or '').startswith(('invalid_', 'undeclared_', 'schema'))) for g in groups)
    if schema_critical:
        return verdict
    ...
    high_groups = sum(1 for g in groups if g.get('severity') == 'high')
    medium_groups = sum(1 for g in groups if g.get('severity') == 'medium')
    if verdict.get('color') == 'red' and critical_groups <= 1:
        floor = 4.8 if high_groups >= 6 else 5.2
        if medium_groups >= 7:
            floor -= 0.3
        verdict['score'] = round(max(float(verdict.get('score') or 0), floor), 1)
        verdict['verdict'] = 'НЕ ГОТОВО: есть production-риски, но схема пригодна для доработки'
    return verdict

# v8.6.59c: do not raise the score floor for deliberately stripped unsafe schemes.
_PREV_SCORE_V8659C = score

def score(findings, finding_groups=None):
    verdict = dict(_PREV_SCORE_V8659C(findings, finding_groups))
    groups = finding_groups or group_findings(findings)
    fired = {str(g.get('rule') or '') for g in groups}
    high_groups = sum(1 for g in groups if g.get('severity') == 'high')
    missing_control_bundle = bool({'missing_timeouts', 'event_core_fields', 'no_correlation_id', 'observability_missing'} & fired)
    no_recovery_or_idem = bool({'async_without_recovery', 'retry_without_idempotency'} & fired)
    dangerous = ('external_blocking' in fired and missing_control_bundle and no_recovery_or_idem) or high_groups >= 8
    schema_critical = any((g.get('severity') == 'critical' and str(g.get('rule') or '').startswith(('invalid_', 'undeclared_', 'schema'))) for g in groups)
    if dangerous and not schema_critical:
        verdict['score'] = round(min(float(verdict.get('score') or 0), 5.0), 1)
        verdict['color'] = 'red'
        verdict['verdict'] = 'НЕ ГОТОВО: отсутствуют базовые production-контроли'
    return verdict
