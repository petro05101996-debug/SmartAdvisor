...
SYNC_CHANNELS = {'rest', 'soap', 'grpc', 'db'}
ASYNC_CHANNELS = {'kafka', 'queue', 'webhook', 'callback', 'file', 'batch', 'cdc'}
...
def normalize(payload):
    ...
    meta = {
        ...
        'customer_visible': _low(meta_in.get('customer_visible')) in ('yes', 'mixed', 'да'),
        'money': _low(meta_in.get('money')) or 'no',            # no|indirect|direct
        'regulatory': _low(meta_in.get('regulatory')) == 'yes',
        ...
        'multi_tenant': _low(meta_in.get('multi_tenant')) == 'yes',
        'replacing_legacy': _low(meta_in.get('replacing_legacy')) == 'yes',
    }
...
def r_dual_write(model):
    ...
    for sysname, ss in by_system.items():
        ...
        publishes = [x for x in ss if x['channel'] in ('kafka', 'queue')
...
def r_ordering(model):
    meta = model['meta']
    kafka_steps = [s for s in model['steps'] if s['channel'] == 'kafka']
...
def r_stream_consumer(model):
    ...
    out = []
    for s in model['steps']:
        if s['channel'] in ('kafka', 'queue') and _is_consumption(model, s):
...
def r_stream_ingestion(model):
    meta = model['meta']
    kafka_async = [s for s in model['steps'] if s['channel'] == 'kafka' and not s['blocking']]
...
def r_multi_tenant(model):
    meta = model['meta']
    has_queue = any(s['channel'] in ('kafka', 'queue') for s in model['steps'])
...
def r_unbounded_growth(model):
    ...
    steps, meta = model['steps'], model['meta']
    has_outbox = any(s['channel'] in ('kafka', 'queue') and not _is_consumption(model, s) for s in steps)
...
def r_contract_versioning(model):
    ...
    steps = model['steps']
    publishes = [s for s in steps if s['channel'] in ('kafka', 'queue')
                 and not _is_consumption(model, s)]
    consumes = [s for s in steps if s['channel'] in ('kafka', 'queue')
...
def completeness_check(model):
    ...
    kafka_or_queue = [s for s in steps if s['channel'] in ('kafka', 'queue')]
...
def recommend_patterns(model, findings):
    ...
    if 'dual_write' in fired or any(s['channel'] in ('kafka', 'queue') for s in model['steps']):
...
def mermaid_flow(model):
    ...
    for s in model['steps']:
        ...
        for src in srcs:
            lines.append(f'  {src} {style}|{s["order"]}. {s["channel"]}| {_mid(s["system"])}')
    return '\n'.join(lines)
...
def mermaid_sequence(model):
    ...
    for s in model['steps']:
        ...
        for src in srcs:
            lines.append(f'  {src}{arrow}{_mid(s["system"])}: {s["order"]}. {s["name"]} [{s["channel"]}]')
    return '\n'.join(lines)
...
def _step_failure_handling(step, system_role='internal'):
    ...
    if ch in ('kafka', 'queue'):
        parts = [
            ...
            'После исчерпания попыток сообщение уходит в DLQ/карантин с причиной ошибки и correlationId.',
...
def development_scenario(model, findings, patterns):
    ...
    for s in steps:
        ...
        main_flow.append({
            'order': s['order'],
            'title': f"{s['system']}: {s['name']}",
            'actor': s['system'],
            'channel': s['channel'],
            'depends_on': deps,
            'what_happens': f"Система «{s['system']}» выполняет действие «{s['name']}» через канал {s['channel']}.",
            'result': f"Результат шага фиксируется в статусе процесса или в журнале шагов; {effect}.",
            'controls': [wait] + ([f"timeout: {s['timeout_ms']} мс"] if s['timeout_ms'] else [])
                        + ([f"повтор: {s['retry']}"] if s['retry'] != 'none' else [])
                        + ([f"идемпотентность: {s['idempotency']}"] if s['idempotency'] != 'none' else []),
