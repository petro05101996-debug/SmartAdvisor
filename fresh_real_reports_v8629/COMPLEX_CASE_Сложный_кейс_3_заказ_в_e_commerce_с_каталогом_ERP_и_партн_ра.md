...
- **Шаг 6 «ERP отдаёт корпоративные сущности через OData»**: Источник и получатель совпадают. Скорее всего, потерян реальный участник связи.
- **Шаг 7 «Заказ сохраняется в основную БД»**: Шаг описан как сохранение результата, но получатель не похож на хранилище данных. Измените действие на «передаёт данные» либо выберите БД/хранилище состояния как получателя.
- **Шаг 9 «Идемпотентный быстрый доступ к корзине идёт по ключу»**: Источник и получатель совпадают. Скорее всего, потерян реальный участник связи.
- **Шаг 10 «Гибкий документ состава заказа хранится как документ»**: Источник и получатель совпадают. Скорее всего, потерян реальный участник связи.
...
**Что:** шаг 6 — «ERP отдаёт корпоративные сущности через OData». Основной способ взаимодействия: сначала исправить схему.
...
**Что:** шаг 7 — «Заказ сохраняется в основную БД». Основной способ взаимодействия: сначала исправить схему.
...
**Почему:** Шаг описан как сохранение результата, но получатель не похож на хранилище данных. Измените действие на «передаёт данные» либо выберите БД/хранилище состояния как получателя.
**Почему не другой вариант:** Выбор gRPC, REST, брокера или БД поверх некорректной связи создаст ложное ощущение готовой архитектуры.
**Что проверить перед выпуском:** Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека.

### Шаг 9. Идемпотентный быстрый доступ к корзине идёт по ключу

**Что:** шаг 9 — «Идемпотентный быстрый доступ к корзине идёт по ключу». Основной способ взаимодействия: сначала исправить схему.
...
**Что:** шаг 10 — «Гибкий документ состава заказа хранится как документ». Основной способ взаимодействия: сначала исправить схему.
...
| 5. Каталог читает редко меняющиеся данные из простого временного кэша | Каталог → GraphQL BFF. Исполнитель: Memcached | GraphQL | Нужны авторизация полей, лимит глубины/сложности запроса, пагинация, защита от тяжёлых запросов и понятный контракт схемы. |
| 6. ERP отдаёт корпоративные сущности через OData | Каталог → Каталог. Исполнитель: OData ERP | сначала исправить схему | Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека. |
| 7. Заказ сохраняется в основную БД | GraphQL BFF → БД заказов. Исполнитель: GraphQL BFF | сначала исправить схему | Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека. |
...
| 9. Идемпотентный быстрый доступ к корзине идёт по ключу | GraphQL BFF → GraphQL BFF. Исполнитель: DynamoDB | сначала исправить схему | Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека. |
| 10. Гибкий документ состава заказа хранится как документ | GraphQL BFF → GraphQL BFF. Исполнитель: MongoDB | сначала исправить схему | Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека. |
...
| 14. В корпоративном Microsoft-контуре заказ передаётся в очередь | GraphQL BFF → OData ERP. Исполнитель: Azure Service Bus | OData | Нужны ограничения фильтров, права на поля, лимит размера ответа, версионирование сущностей и аудит доступа. |
...
|---|---|---|---|
| API и онлайн-взаимодействие | шаг 1: Web/Mobile → Каталог — Клиент получает каталог через гибкий API (GraphQL); шаг 5: Каталог → GraphQL BFF — Каталог читает редко меняющиеся данные из простого временного кэша (GraphQL); шаг 14: GraphQL BFF → OData ERP — В корпоративном Microsoft-контуре заказ передаётся в очередь (OData) | входящий или внутренний запрос получает быстрый ответ либо идентификатор отслеживания для дальнейшего отслеживания | Нужны авторизация полей, лимит глубины/сложности запроса, пагинация, защита от тяжёлых запросов и понятный контракт схемы; Нужны авторизация полей, лимит глубины/сложности запроса, пагинация, защита от тяжёлых запросов и понятный контракт схемы; ещё 1 |
| Асинхронный обмен | шаг 11: GraphQL BFF → Каталог — Короткая внутренняя задача пересчитывает рекомендации (Redis как короткая очередь задач); шаг 12: GraphQL BFF → Каталог — Если брокер ещё не выбран, задача уходит в нейтральную очередь (Очередь сообщений, брокер пока не выбран); шаг 13: GraphQL BFF → Kafka — Событие о заказе публикуется многим потребителям (Kafka) | нагрузка выровнена через брокер, сообщение не теряется, есть повторная обработка и очередь ошибок | Нужны TTL, повторные попытки, обработка зависших задач и понимание риска потери при неверной настройке сохранность данных; Нужно отдельно выбрать брокер, определить модель подтверждения, лимит повторов, очередь ошибок и владельца разбора; ещё 1 |
| Данные и чтение | шаг 3: GraphQL BFF → Web/Mobile — Поиск товара идёт по поисковому индексу (Поисковый индекс); шаг 4: GraphQL BFF → Web/Mobile — Семантический поиск похожих товаров использует векторы (Векторное хранилище); шаг 8: БД заказов → Шардированное хранилище — Данные заказов разделяются по ключу клиента (Шардирование базы данных) | основная сущность, справочники или проекции сохранены без дублей и потерянных обновлений | Нужны переиндексация, контроль отставания индекса, правила актуализации и понятная свежесть данных для пользователя; Нужны модель эмбеддингов, версия векторов, переиндексация, контроль качества поиска и правила доступа к исходным текстам; ещё 1 |
| Файлы и доставка контента | шаг 15: Файловый шлюз → SFTP партнёра — Партнёр получает защищённый файл по SFTP (SFTP); шаг 16: Файловый шлюз → SFTP партнёра — Одиночный файл накладной передаётся отдельным файлом (SFTP); шаг 17: GraphQL BFF → Object Storage — Большие вложения заказа хранятся отдельно (Объектное хранилище) | файлы, документы и статический контент передаются с контролем доступа, размера и целостности | Нужны имя файла, контрольная сумма, идентификатор пакета, журнал загрузки, карантин ошибок и повторная обработка файла; Нужны имя файла, контрольная сумма, идентификатор пакета, журнал загрузки, карантин ошибок и повторная обработка файла; ещё 1 |
| Прочее | шаг 6: Каталог → Каталог — ERP отдаёт корпоративные сущности через OData (сначала исправить схему); шаг 7: GraphQL BFF → БД заказов — Заказ сохраняется в основную БД (сначала исправить схему); шаг 9: GraphQL BFF → GraphQL BFF — Идемпотентный быстрый доступ к корзине идёт по ключу (сначала исправить схему); ещё 1 | дополнительные взаимодействия описаны отдельно и требуют ручного уточнения владельцев и результата | Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека; Верните шаг на этап проектирования связей, исправьте источник, получателя и смысл действия, затем повторно запустите проверку и подбор стека; ещё 2 |

### Сквозные сценарии контроля

| Контроль | Где применяется | Ожидаемый результат |
|---|---|---|
| Статика и изображения товаров отдаются быстро | CDN → Web/Mobile; при необходимости применяется ко всему процессу | контроль работает сквозно и не меняет порядок бизнес-шагов |
...
flowchart LR
 ...
 N2 -->|"5. Каталог читает редко меняющиеся данные из прост… / GraphQL"| N3
 N2 -->|"6. ERP отдаёт корпоративные сущности через OData / сначала исправить схему"| N4
 N3 -->|"7. Заказ сохраняется в основную БД / сначала исправить схему"| N5
 N5 -->|"8. Данные заказов разделяются по ключу клиента / Шардирование базы данных"| N6
 N3 -->|"9. Идемпотентный быстрый доступ к корзине идёт по… / сначала исправить схему"| N7
 N3 -->|"10. Гибкий документ состава заказа хранится как док… / сначала исправить схему"| N8
 ...
 N3 -.->|"14. В корпоративном Microsoft-контуре заказ передаё… / OData"| N4
...
sequenceDiagram
 ...
 N2->>N3: 5. Каталог читает редко меняющиеся данные из прост… / GraphQL
 N2->>N4: 6. ERP отдаёт корпоративные сущности через OData / сначала исправить схему
 N3->>N5: 7. Заказ сохраняется в основную БД / сначала исправить схему
 N5->>N6: 8. Данные заказов разделяются по ключу клиента / Шардирование базы данных
 N3->>N7: 9. Идемпотентный быстрый доступ к корзине идёт по… / сначала исправить схему
 N3->>N8: 10. Гибкий документ состава заказа хранится как док… / сначала исправить схему
 ...
 N3--)N4: 14. В корпоративном Microsoft-контуре заказ передаё… / OData
