...
**Что:** шаг 10 — «Распределённая обработка пересчитывает большие периоды». Основной способ взаимодействия: ETL/ELT-загрузка.
...
| 10. Распределённая обработка пересчитывает большие периоды | Lakehouse → Data Warehouse. Исполнитель: Spark | ETL/ELT-загрузка | Нужны правила преобразования, контроль количества записей, журнал загрузки, карантин ошибок, повторный запуск периода и сверка с источником. |
...
flowchart LR
 ...
 N10 -.->|"10. Распределённая обработка пересчитывает большие… / ETL/ELT-загрузка"| N11
...
sequenceDiagram
 ...
 N10--)N11: 10. Распределённая обработка пересчитывает большие… / ETL/ELT-загрузка
...
Матрица деталей: применимо инвариантов из каталога v7.1 — 117 из 125; блокируют выпуск — 12, требуют внимания — 32, нужно уточнить — 49, уже выглядит закрытым — 39.
