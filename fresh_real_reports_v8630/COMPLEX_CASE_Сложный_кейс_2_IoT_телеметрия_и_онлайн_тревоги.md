...
| 4 | Сервис тревог → Сервис тревог | Лёгкая внутренняя рассылка уведомляет сервисы о тревоге | сообщение принято в брокер, дальнейшая обработка идёт асинхронно, статус процесса отслеживается отдельно |
...
| 10 | ClickHouse → ClickHouse | Аналитические модели собираются управляемо | данные доставлены в аналитический контур, полнота загрузки подтверждается сверкой |
