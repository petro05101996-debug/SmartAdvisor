...
| 11. Короткая внутренняя задача пересчитывает рекомендации | GraphQL BFF → Каталог. Исполнитель: Redis queue | Redis как короткая очередь задач | Нужны TTL, повторные попытки, обработка зависших задач и понимание риска потери при неверной настройке сохранность данных. |
...
| 6 | Каталог → Каталог | ERP отдаёт корпоративные сущности через OData | получен ответ или понятная ошибка с кодом, таймаутом и идентификатором сквозной связи |
...
| 9 | GraphQL BFF → GraphQL BFF | Идемпотентный быстрый доступ к корзине идёт по ключу | состояние или данные сохранены с защитой от дублей и конкурентных изменений |
| 10 | GraphQL BFF → GraphQL BFF | Гибкий документ состава заказа хранится как документ | состояние или данные сохранены с защитой от дублей и конкурентных изменений |
