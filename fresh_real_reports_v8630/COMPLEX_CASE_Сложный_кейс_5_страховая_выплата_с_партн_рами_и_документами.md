...
| 6 | Сервис страховой заявки → Сервис страховой заявки | OCR получает фоновую задачу | сообщение принято в брокер, дальнейшая обработка идёт асинхронно, статус процесса отслеживается отдельно |
...
| 8 | Сервис страховой заявки → Сервис страховой заявки | Нельзя одновременно менять одну выплату | шаг завершён, результат отражён в статусе процесса или журнале шагов |
...
**Когда возникает:** Техническая доставка прошла не полностью, повторная обработка былаааа пропущена или потребитель отстал.
**Как должен пройти сценарий:**
...
flowchart LR
 ...
 N3 -.->|"15. События выплаты читают уведомления, аудит и ана… / Kafka"| N5
