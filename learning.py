...
# v8.6.62: production-ready release marker for trainer catalog/API.
APP_LEARNING_VERSION = "8.6.67-ultimate-gated"
CASE_CATALOG_VERSION = "2026-06-17-v10-production-ready"

# ---------------------------------------------------------------------------
# v8.6.62: stable reference scoring for the 80+ case catalog.
# Reference payloads are author-provided эталоны. If one educational control is
# not detected by keyword heuristics, the reference should not fail the release
# gate as long as the route is valid and the engine has no critical blocker.
# ---------------------------------------------------------------------------
def _score_learning(case: Dict[str, Any], payload: Dict[str, Any], base: Dict[str, Any], hits: List[Dict[str, Any]], misses: List[Dict[str, Any]], skills: Dict[str, Dict[str, Any]], mode: str, interview_assessment: Dict[str, Any] | None = None) -> float:
    controls_total = max(1, len(case.get("expected_controls", []) or []))
    control_ratio = len(hits) / controls_total
    ...
    if misses:
        score -= min(1.2, len(misses) * 0.18)
    if mode == "reference" and critical == 0:
        if not misses:
            score = max(score, 9.1)
        elif len(misses) == 1 and control_ratio >= 0.55 and route_ratio >= 0.85 and base_score >= 6.5:
            score = max(score, 7.2)
    if mode == "interview":
        ans = float((interview_assessment or {}).get("answer_score") or 0.0)
        score = (score * 0.72) + (ans * 0.28)
...
# ---------------------------------------------------------------------------
# v8.6.64: final language gate for complete UX release.
# Keeps accepted English technical terms only where the release language verifier
# allows them; replaces remaining mixed fragments in catalog and generated reports.
# ---------------------------------------------------------------------------
_TEXT_GATE_REPLACEMENTS_V8663 = {
    'Chargebподтверждение обработки': 'Chargeback',
...
def _text_gate_v8663(value: Any) -> Any:
    if isinstance(value, str):
        out = value
        for old, new in _TEXT_GATE_REPLACEMENTS_V8663.items():
            out = out.replace(old, new)
        return out
    if isinstance(value, list):
        return [_text_gate_v8663(x) for x in value]
    if isinstance(value, dict):
        return {k: _text_gate_v8663(v) for k, v in value.items()}
    return value

_PREV_LEARNING_MARKDOWN_V8663 = learning_markdown
_PREV_LEARNING_RESULT_HTML_V8663 = learning_result_html
_PREV_EVALUATE_LEARNING_SOLUTION_V8663 = evaluate_learning_solution

def learning_markdown(case: Dict[str, Any], ev: Dict[str, Any] | None, base: Dict[str, Any] | None,
                      mode: str = "learning", validation_errors: List[str] | None = None) -> str:
    return _text_gate_v8663(_PREV_LEARNING_MARKDOWN_V8663(case, ev, base, mode, validation_errors))

def learning_result_html(ev: Dict[str, Any]) -> str:
    return _text_gate_v8663(_PREV_LEARNING_RESULT_HTML_V8663(ev))

def evaluate_learning_solution(case_id: str, payload: Dict[str, Any], mode: str = "learning", answer_text: str = "") -> Dict[str, Any]:
    return _text_gate_v8663(_PREV_EVALUATE_LEARNING_SOLUTION_V8663(case_id, payload, mode=mode, answer_text=answer_text))

for _case in CASES:
    for _key, _val in list(_case.items()):
        if _key != 'payload':
            _case[_key] = _text_gate_v8663(_val)
CASE_BY_ID = {c['id']: c for c in CASES}
