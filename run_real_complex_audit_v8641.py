...
# v8.6.57: быстрый smoke без запуска фиксированного порта.
# Реальный HTTP/API уже покрывает verify_saas_ui_v8654.py; здесь важнее,
# чтобы тяжёлый audit-скрипт не зависал в CI/контейнере.
def live_api_smoke(case_payloads):
    out = []
...
if __name__ == '__main__':
    ...
    # v8.6.57: подробные элементы лежат в JSON-файле; stdout оставляем коротким,
    # чтобы audit не зависал/не раздувал CI-log на тяжёлых отчётах.
    if channel_issues:
