...
def test_builder_navigation_and_buttons():
    ...
    with sync_playwright() as p:
        ...
        # В начале пользователь видит смысловые вопросы, но не выбирает конкретный стек.
        expect(page.get_by_role("heading", name="1. Соберите цепочку из универсальных вариантов")).to_be_visible()
        expect(page.get_by_text("Сначала смысл процесса, потом стек")).to_be_visible()

        # Проходим реальный путь пользователя через мастер.
        for group, value in [
        ...
        expect(page.locator("#chainList .chain-component").first).to_be_visible()
        expect(page.get_by_text("стек ещё не сформирован").first).to_be_visible()

        # Добавляем смысловые уточнения, не выбирая технологии руками.
        ...
        expect(page.locator("#moduleStatus")).to_contain_text("Быстрый доступ")
        ...
        expect(page.get_by_text("Стек подбирается автоматически").first).to_be_visible()
        ...
        page.locator('[data-action="insert-after"]').first.click()
        page.locator('[data-action="safe-step"]').first.click()
        ...
        assert len(payload.get("systems", [])) >= 4
...
def test_mobile_layout_and_reference_cards_with_set_content():
    ...
    with sync_playwright() as p:
        ...
        for group, value in [
            ...
            page.locator(f'[data-action="compose-choice"][data-compose-group="{group}"][data-value="{value}"]').click()
        page.get_by_role("button", name="Собрать цепочку").click()
        page.locator('[data-module="dwh"]').click()
        page.locator('[data-module="manual_recon"]').click()
        expect(page.locator("#chain-builder")).to_be_visible()
        expect(page.locator("#moduleStatus")).to_contain_text("Аналитическое хранилище")
        assert page.locator(".chain-component").count() >= 6
