...
def test_start_screen_has_only_three_main_actions(page, app_server):
    ...
    assert start.get_by_role("button", name="Начать проектирование").is_visible()
    assert start.get_by_role("button", name="Проверить существующее решение").is_visible()
    assert start.get_by_role("button", name="Расширенный режим").is_visible()
    assert start.locator("button").count() == 3
