...
APP_VERSION = '8.6.18-ultra-report-logic-fixed'
  ...
  const cards=state.steps.map((step,idx)=>{
    ...
    const groups=ids.map(id=>{
      ...
      const items=g.items.map(([module,label,hint])=>{
        ...
        return '<button type="button" class="branch-question-btn '+(hasModule(module)?'active':'')+'" data-action="module" data-module="'+esc(attrModule)+'" data-module-kind="'+esc(module)+'" data-step-index="'+idx+'" data-step-label="'+esc(routeLabel(step,idx))+'"><b>'+esc(label)+'</b><small>'+esc(hint)+'</small></button>';
      }).join('');
      return '<details class="branch-question-group contextual" open><summary><h4>'+esc(g.title)+'</h4></summary><p><b>Относится к связи:</b> '+esc((step.source_system||'Источник')+' → '+(step.target_system||'Получатель'))+'. '+esc(g.help)+'</p><div class="branch-question-grid">'+items+'</div></details>';
    }).join('');
...
function renderModules(){
  document.querySelectorAll('[data-action="module"]').forEach(btn=>{const k=btn.dataset.moduleKind||btn.dataset.module; btn.classList.toggle('active',state.modules.some(m=>m.kind===k));});
  ...
  const filled='Добавлены уточнения:<br>'+state.modules.map(m=>'<b>'+esc(m.label)+'</b>'+((m.contexts&&m.contexts.length)?'<small> — '+esc(m.contexts.join('; '))+'</small>':'')).join('<br>')+'<br><span class="hint">Эти ответы сужают выбор стека и будут отражены в отчёте: что выбрано, где применяется и почему не другой вариант.</span>';
