...
APP_VERSION = '8.4.7-auto-route-on-move'
...
function findKafka(){return state.systems.find(s=>s.role==='broker'||/kafka|broker|топик/i.test(s.name||''))?.name||'Kafka';}
...
const MODULE_LABELS={dwh:'DWH/аналитика',legacy:'Legacy-потребитель',manual_recon:'Ручная сверка',enrichment:'REST-обогащение',fanin:'Fan-in / join',retry_dlq:'Retry/DLQ/replay',audit:'Аудит/регуляторика',outbox_inbox:'Outbox/Inbox',contract:'Миграция контракта',security:'ПДн/security'};
...
function composeChainFromChoices(){
  ...
  if(['event'].includes(composerDraft.start) || ['later','both'].includes(composerDraft.timing) || composerDraft.activity==='wait_status') ensureSystem('Kafka / очередь','broker');
  ...
  if(['later','both'].includes(composerDraft.timing) || composerDraft.activity==='wait_status'){
    add({name:'Принять результат или статус позже',source_system:provider,system:provider,target_system:'Kafka / очередь',channel:'kafka',blocking:'no',retry:'auto',idempotency:'key',depends_on:String(state.steps.length),compensation:'повторная публикация, DLQ, eventId, statusVersion'});
    add({name:'Дедуплицировать поздний результат и обновить историю',source_system:'Kafka / очередь',system:proc,target_system:db,channel:'kafka',blocking:'no',retry:'auto',idempotency:'key',writes_entity:'yes',depends_on:String(state.steps.length),compensation:'Inbox, UNIQUE eventId, replay-safe update, status history'});
...
function roleGuessFor(name,channel){const n=normTxt(name); if(!name)return 'internal'; if(channel==='db'||n.includes('бд')||n.includes('postgres')||n.includes('database'))return 'db'; if(channel==='kafka'||channel==='queue'||n.includes('kafka')||n.includes('очеред')||n.includes('broker'))return 'broker'; if(n.includes('dwh')||n.includes('витрин')||n.includes('audit mart'))return 'analytics'; if(n.includes('legacy'))return 'legacy'; if(n.includes('внешн')||n.includes('поставщик')||n.includes('провайдер')||n.includes('ук')||n.includes('справочник')||n.includes('получатель'))return 'external'; return 'internal';}
...
function autoFillRouteForStepAt(i){const s=state.steps[i]; if(!s)return; const proc=firstSystemByRole('internal','Сервис процесса'); const db=firstSystemByRole('db','БД процесса'); const broker=findKafka(); const external=firstSystemByRole('external','Внешняя система / поставщик'); const analytics=firstSystemByRole('analytics','DWH / audit mart'); const prevOut=previousOutput(i);
  ...
  else if(nameHas(s,['дедуплицировать','inbox','принять результат или статус позже','принять входящее событие'])){s.source_system=s.source_system||broker; s.system=proc; s.target_system=db; if(['kafka','queue'].includes(s.channel)){s.blocking='no'; s.idempotency='key';}}
  ...
  else if(s.channel==='kafka'||nameHas(s,['опубликовать','событие'])){s.source_system=prevOut; s.system=s.system||proc; s.target_system=broker; s.channel='kafka'; s.blocking='no'; s.idempotency='key';}
  ...
  s.depends_on=semanticDependsForStep(s,i); ensureRouteSystemsForStep(s);
}
function normalizeChainAfterStructureChange(opts={}){state.steps.forEach((s,i)=>{s.order=i+1;}); if(opts.autofillRoutes){state.steps.forEach((_,i)=>autoFillRouteForStepAt(i));} repairStepDependencies(); state.steps.forEach(ensureRouteSystemsForStep);}
...
<section class="hero">
 ...
 <p>Основной путь теперь без ручного ввода и без стартовых фраз-шаблонов: выберите, как процесс начинается, что надо сделать, когда приходит результат и что с ним делать. Система сама соберёт черновую цепочку, а затем её можно усложнять признаками процесса.</p>
 <div class="steps3"><div class="stepbox"><b>1. Собрать смысл</b><br>Выбираем из вариантов, что делает система: получить, сохранить, передать, дождаться.</div><div class="stepbox"><b>2. Усложнить слоями</b><br>Отмечаем дубли, статусы, DWH, legacy, join, ПДн, деньги или highload.</div><div class="stepbox"><b>3. Проверить архитектуру</b><br>Даже поверхностный ввод даст черновой анализ с явными допущениями.</div></div>
 <div class="quickstart">
  <div class="quickstart-card">
   <h3>Пример входа до заполнения</h3>
   <div class="example-flow"><div class="ex"><b>Ситуация</b>Пользователь знает только общий смысл процесса и не уверен в деталях.</div><div class="ex"><b>Что выбирает пользователь</b>Старт, действие, характер результата, действие с результатом и примерное число систем.</div><div class="ex"><b>Что подставляет интерфейс</b>Систему процесса, БД, статусы, ключи, retry, idempotency, correlationId и recovery.</div></div>
  </div>
  <div class="quickstart-card">
   <h3>Что получится на выходе</h3>
   <p>Риски, production-gates, паттерны, DDL, сценарии разработки, тесты, Mermaid-диаграммы и список уточнений. Пользователь не обязан заранее знать все технические поля.</p>
   <button type="button" class="btn ghost" data-action="compose-chain">собрать черновик из вариантов</button>
  </div>
 </div>
 <div class="navlinks"><a href="{url_for('/invariants')}">Открыть справочник инвариантов</a><a href="{url_for('/patterns')}">Открыть шаблоны проектирования</a><a href="{url_for('/')}">Собрать процесс</a></div>
</section>
 ...
 <div class="composer">
  <div class="composer-panel wizard-shell">
   ...
   <div class="wizard-pane active" data-wizard-pane="start"><h3>Как начинается процесс?</h3><div class="wizard-choice-count">Выберите один вариант. Следующий вопрос откроется автоматически.</div><div class="choice-grid">
    ...
    <button type="button" class="choice-card compact" data-action="compose-choice" data-compose-group="start" data-value="event"><b>Событие из очереди</b><small>Kafka/очередь/топик</small></button>
    <button type="button" class="choice-card compact" data-action="compose-choice" data-compose-group="start" data-value="file"><b>Файл или batch</b><small>пакетная обработка</small></button>
    <button type="button" class="choice-card compact" data-action="compose-choice" data-compose-group="start" data-value="schedule"><b>По расписанию</b><small>job/регламент</small></button>
   ...
   <div class="wizard-pane" data-wizard-pane="activity"><h3>Что нужно сделать?</h3><div class="choice-grid">
    <button type="button" class="choice-card compact active recommended" data-action="compose-choice" data-compose-group="activity" data-value="call_external"><b>Вызвать внешнюю систему</b><small>API/сервис/провайдер</small></button>
   ...
   <div class="wizard-pane" data-wizard-pane="timing"><h3>Когда появляется результат?</h3><div class="choice-grid">
    ...
    <button type="button" class="choice-card compact" data-action="compose-choice" data-compose-group="timing" data-value="later"><b>Позже</b><small>callback/webhook/event</small></button>
   ...
   <div class="wizard-pane" data-wizard-pane="result"><h3>Что делать с результатом?</h3><div class="choice-grid">
    <button type="button" class="choice-card compact" data-action="compose-choice" data-compose-group="result" data-value="save"><b>Сохранить</b><small>БД/история/статус</small></button>
    ...
    <button type="button" class="choice-card compact" data-action="compose-choice" data-compose-group="result" data-value="compare"><b>Сверить/сравнить</b><small>reconciliation</small></button>
  ...
  <div class="composer-panel">
   ...
   <p>После 5 коротких выборов появится минимальная цепочка. Затем можно добавить production-признаки: дубли, DWH, legacy, ручная сверка, join, ПДн, highload.</p>
   ...
   <div class="expert-link-note">REST/Kafka/Outbox/Inbox не выбираются на старте — они появятся как технические следствия выбранных вариантов и усложнений.</div>
  </div>
 </div>
 <details class="template-fold">
  ...
  <div class="scenario-groups">
   <div class="scenario-group"><h3>Частые бизнес-процессы</h3><p>Это не обязательные шаблоны, а примеры для быстрого заполнения.</p><div class="scenarios">
    <button type="button" class="scenario featured" data-action="scenario" data-scenario="reverse" aria-pressed="false"><b>Получить статусы от внешней системы</b><span>Банк ↔ УК, документы, операции, обратный поток статусов.</span><small>REST + Kafka + история статусов</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="payment" aria-pressed="false"><b>Платёж / деньги / подтверждение</b><span>Создание операции, блокировка денег, внешний провайдер, финальный статус.</span><small>ledger + idempotency + audit</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="order" aria-pressed="false"><b>Заказ товара / выполнение заявки</b><span>Заказ, склад, доставка, уведомления и отмена.</span><small>saga + статусы + компенсации</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="external_sync" aria-pressed="false"><b>Вызвать внешний сервис и ждать ответ</b><span>Синхронный REST/gRPC-вызов в критическом пути.</span><small>timeout + fallback + circuit breaker</small></button>
   </div></div>
   <div class="scenario-group"><h3>Интеграции и события</h3><p>Для Kafka, webhook, batch, CDC и обогащения данных.</p><div class="scenarios">
    <button type="button" class="scenario" data-action="scenario" data-scenario="kafka" aria-pressed="false"><b>Отправить изменение через Kafka</b><span>Публикация изменений, Outbox, Inbox, DLQ.</span><small>event-driven</small></button>
    <button type="button" class="scenario featured" data-action="scenario" data-scenario="enrichment" aria-pressed="false"><b>Обогатить событие через REST</b><span>Kafka + REST-справочник + enriched-event.</span><small>компромисс без нового брокера</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="webhook_in" aria-pressed="false"><b>Принять webhook / callback</b><span>Внешняя система сама присылает результат.</span><small>signature + duplicate callback</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="batch_file" aria-pressed="false"><b>Передать файл / batch</b><span>Пакетная выгрузка, checksum, карантин и повторная обработка.</span><small>batchId + reconciliation</small></button>
   </div></div>
   <div class="scenario-group"><h3>Сложные/технические кейсы</h3><p>Когда важны нагрузка, аналитика, legacy или универсальные сервисы.</p><div class="scenarios">
    <button type="button" class="scenario" data-action="scenario" data-scenario="dwh" aria-pressed="false"><b>Выгрузить данные в аналитику</b><span>CDC/ETL/DWH без нагрузки на core-flow.</span><small>watermark + backfill</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="highload" aria-pressed="false"><b>Highload consumer</b><span>Фильтрация общего Kafka-топика и запись в Postgres.</span><small>lag + backpressure</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="dispatcher" aria-pressed="false"><b>Универсальный докатчик</b><span>Один operUid, разные operationType и целевые системы.</span><small>composite key</small></button>
    <button type="button" class="scenario" data-action="scenario" data-scenario="legacy_migration" aria-pressed="false"><b>Заменить legacy-процесс</b><span>Dual-run, cutover, откат и незавершённые операции.</span><small>migration gates</small></button>
   </div></div>
   <div class="scenario-group"><h3>Ультрасложные заготовки</h3><p>Заготовки для нескольких потоков, join, регуляторных изменений и совместимости.</p><div class="scenarios">
    <button type="button" class="scenario featured" data-action="scenario" data-scenario="reverse_split" aria-pressed="false"><b>Документы + операции в разных потоках</b><span>Два обратных потока, join, отставание партиций, reconciliation.</span><small>fan-in + сверка + manual recovery</small></button>
...
<section class="card" id="complexity-modules"><h2>2. Усложнить процесс слоями</h2>
 <p class="human-tip">Не выбирайте технические паттерны вручную. Отметьте человеческие признаки процесса — UI сам добавит шаги, ключи, статусы, recovery и ограничения.</p>
 <div class="question-grid">
  <div class="question-card"><b>Есть параллельные ветки?</b><span>Нажмите Fan-in / join — появятся две ветки, окно ожидания и единое решение.</span></div>
  <div class="question-card"><b>Есть DWH, legacy или ручная сверка?</b><span>Нажмите соответствующую надстройку — добавятся CDC, dual-run, reconciliation или manual review.</span></div>
 </div>
 <div class="module-panel module-groups">
  <div class="module-group"><h3>Надёжность</h3><div class="module-grid">
   <button type="button" class="module-btn" data-action="module" data-module="retry_dlq"><b>Могут быть ошибки и повторы</b><small>Ограниченный retry, poison messages, replay по ключу.</small></button>
   <button type="button" class="module-btn" data-action="module" data-module="outbox_inbox" data-legacy-label="+ Outbox/Inbox"><b>Могут быть дубли</b><small>Transactional outbox, входящая дедупликация, replay-safe.</small></button>
   <button type="button" class="module-btn" data-action="module" data-module="manual_recon"><b>Нужна ручная сверка</b><small>Расхождения, ожидание парной ветки, runbook, replay.</small></button>
  </div></div>
  <div class="module-group"><h3>Потоки и системы</h3><div class="module-grid">
   <button type="button" class="module-btn" data-action="module" data-module="fanin"><b>Нужно дождаться нескольких веток</b><small>Параллельные проверки, depends_on 2,3, компенсации.</small></button>
   <button type="button" class="module-btn" data-action="module" data-module="enrichment"><b>Нужно сходить в справочник</b><small>Внешний справочник, cache/fallback, enriched-event.</small></button>
   <button type="button" class="module-btn" data-action="module" data-module="legacy"><b>Есть legacy-система</b><small>Dual-run, backward compatibility, cutover/rollback.</small></button>
  </div></div>
  <div class="module-group"><h3>Данные и аналитика</h3><div class="module-grid">
   <button type="button" class="module-btn" data-action="module" data-module="dwh" data-legacy-label="+ DWH/аналитика"><b>Есть DWH/аналитика</b><small>CDC, watermark, backfill, reconciliation report.</small></button>
   <button type="button" class="module-btn" data-action="module" data-module="contract"><b>Меняется контракт/модель</b><small>v1/v2, contract tests, canary, rollback.</small></button>
  </div></div>
  <div class="module-group"><h3>Бизнес-риск</h3><div class="module-grid">
   <button type="button" class="module-btn" data-action="module" data-module="audit"><b>Есть деньги/регуляторика</b><small>Append-only audit journal, traceability, retention.</small></button>
   <button type="button" class="module-btn" data-action="module" data-module="security"><b>Есть ПДн/security</b><small>Классификация, маскирование, RBAC, журнал доступа.</small></button>
  </div></div>
...
<section class="card" id="chain-builder"><div class="section-head"><div><h2>3. Проверьте готовую цепочку</h2><p class="hint">После автосборки здесь появляется процесс. В простом режиме пользователь видит смысловую цепочку. В экспертном режиме можно двигать элементы; поля «откуда / кто выполняет / куда / канал / зависимости» автоматически пересчитываются по месту шага в цепи.</p></div><div class="builder-actions advanced-only"><button type="button" class="btn ghost" data-action="safe-all">безопасные настройки всем шагам</button></div></div>
 <div class="builder-layout">
  <aside class="component-palette"><h3>Быстро добавить типовой шаг</h3><div class="palette-grid">
   <button type="button" class="palette-btn" data-action="template" data-template="rest" data-legacy-label="+ REST-вызов"><b>Вызвать систему и ждать ответ</b><small>Синхронный API, timeout, retry, fallback.</small></button>
   <button type="button" class="palette-btn" data-action="template" data-template="kafka" data-legacy-label="+ Kafka-событие"><b>Отправить событие, результат будет позже</b><small>Асинхронное событие, DLQ, replay, Inbox.</small></button>
   <button type="button" class="palette-btn" data-action="template" data-template="db" data-legacy-label="+ запись в БД"><b>Сохранить данные/статус</b><small>Транзакция, unique key, optimistic locking.</small></button>
   <button type="button" class="palette-btn" data-action="template" data-template="webhook"><b>Принять результат позже</b><small>Подпись, timestamp, дедупликация.</small></button>
   <button type="button" class="palette-btn" data-action="template" data-template="batch"><b>Передать файл/пакет</b><small>batchId, checksum, quarantine, reprocess.</small></button>
   <button type="button" class="palette-btn" data-action="template" data-template="cdc" data-legacy-label="+ CDC"><b>Передать изменения в аналитику</b><small>LSN/offset, watermark, replay/resync.</small></button>
   <button type="button" class="palette-btn" data-action="template" data-template="manual"><b>Отправить на ручной разбор</b><small>Ручной разбор, runbook, оператор.</small></button>
...
<section class="card">
 <h2>Как пользоваться</h2>
 <p>Открывайте карточку нужного паттерна или ищите по словам: <b>outbox</b>, <b>inbox</b>, <b>DLQ</b>, <b>replay</b>, <b>expand</b>, <b>contract</b>, <b>saga</b>, <b>Kafka key</b>, <b>schema registry</b>.</p>
 <h3>Критичные паттерны, которые чаще всего забывают</h3>
 <div class="top10">{top_cards}</div>
 <div class="refbar">
  <input id="pat_q" placeholder="Поиск по паттернам: Outbox, Saga, Expand/Contract, Kafka key...">
...
def result_page(rid, res):
 ...
 <a href="{url_for('/run/' + rid + '.md')}">скачать полный Markdown-отчёт</a> · <a href="{url_for('/')}">начать новый разбор</a> · <a href="{url_for('/invariants')}">справочник инвариантов</a> · <a href="{url_for('/patterns')}">шаблоны проектирования</a></p>
</div>""")
    meta_notes = []
    if m.get('goal'):
