...
function addStep(p={}){
  ...
  tr.innerHTML=`
   ...
   <td class="row-actions"><button type="button" title="Выше" data-row-action="move-up">↑</button><button type="button" title="Ниже" data-row-action="move-down">↓</button><button type="button" title="Копировать шаг" data-row-action="duplicate-step">⧉</button><button type="button" class="safe-btn" title="Подставить безопасные настройки для канала" data-row-action="safe-step">✓</button><button type="button" aria-label="Удалить шаг" data-row-action="delete-step">×</button></td>`;
  tb.appendChild(tr);
...
function addSystem(p={}){
  ...
  tr.innerHTML=`
   ...
   <td style="width:40px"><button type="button" class="btn ghost" aria-label="Удалить систему" data-row-action="delete-system">×</button></td>`;
  tb.appendChild(tr);
...
function resetScenarioSelection(){
  document.querySelectorAll('.scenario').forEach(b=>{b.classList.remove('active');b.setAttribute('aria-pressed','false')});
  const selected=document.getElementById('selectedScenario');
  if(selected) selected.textContent='Сценарий ещё не выбран.';
}
...
function selectScenario(kind, button){
  applyScenario(kind);
  ...
  const label=button?.querySelector('b')?.textContent || 'Сценарий';
  const selected=document.getElementById('selectedScenario');
  if(selected) selected.textContent='Выбран сценарий: '+label+'. Теперь проверьте основную сущность, ключ поиска, статусы и цепочку шагов.';
  showToast(kind==='blank'?'Форма очищена. Заполните процесс с нуля.':'Сценарий подставлен. Проверьте смысл процесса, системы и шаги.');
  if(kind!=='blank') document.getElementById('basics')?.scrollIntoView({behavior:'smooth',block:'start'});
}
...
document.addEventListener('click', function(event){
  ...
  if(action==='scenario'){selectScenario(el.dataset.scenario, el);return;}
  if(action==='mode'){setMode(el.dataset.mode);return;}
  if(action==='clear'){selectScenario('blank', document.querySelector('[data-scenario="blank"]'));return;}
  if(action==='add-system'){addSystem();return;}
  if(action==='add-step'){addStep();return;}
  if(action==='template'){addTemplate(el.dataset.template);return;}
  if(action==='safe-all'){applySafeDefaultsAll();showToast('Безопасные настройки применены ко всем шагам.');return;}
  if(action==='suggest-basics'){suggestBasics();showToast('Базовые статусы, поля и ключи подставлены.');return;}
  if(action==='demo'){demo();return;}
  if(action==='submit'){submitForm();return;}
});
document.addEventListener('click', function(event){
  const btn=event.target.closest('[data-row-action]');
  if(!btn) return;
  const action=btn.dataset.rowAction;
  if(action==='move-up') moveStep(btn,-1);
  if(action==='move-down') moveStep(btn,1);
  if(action==='duplicate-step') duplicateStep(btn);
  if(action==='safe-step') applySafeDefaults(btn);
  if(action==='delete-step'){btn.closest('tr')?.remove();renumberSteps();}
  if(action==='delete-system'){btn.closest('tr')?.remove();syncSys();updateProgress();}
});
async function submitForm(){
  ...
  try{
    const r=await fetch(appUrl('/api/analyze'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const d=await r.json();
    if(d.ok){location.href=appUrl('/run/'+d.id)}else{(d.errors||['Неизвестная ошибка']).forEach(e=>{const el=document.createElement('div');el.className='err';el.textContent=e;box.appendChild(el)})}
...
function highlightNav(selector){
  ...
  const obs=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){links.forEach(a=>a.classList.remove('active'));byId.get(entry.target.id)?.classList.add('active');}})},{rootMargin:'-96px 0px -65% 0px',threshold:0.01});
  byId.forEach((_,id)=>{const node=document.getElementById(id);if(node)obs.observe(node)});
}
...
def form_page():
    body = titleblock('КОНСТРУКТОР ПРОЦЕССА · ПРОВОДНИК ПО ИНТЕГРАЦИИ', active='builder') + f"""
<section class="hero">
...
<section class="card">
 <div class="toolbar">
  <div class="mode" aria-label="Режим заполнения"><button type="button" data-action="mode" data-mode="quick">Быстрый режим</button><button type="button" data-action="mode" data-mode="advanced">Расширенный режим</button></div>
...
<section class="card" id="scenario"><h2>Выберите ближайший сценарий</h2>
 <div class="scenarios">
  ...
  <button type="button" class="scenario" data-action="scenario" data-scenario="enrichment" aria-pressed="false"><b>Обогащение данных</b><span>Kafka + REST к внешней системе + новый топик.</span></button>
...
<section class="card invariant-entry">
 <h2>Справочник инвариантов</h2>
 <p>Откройте список архитектурных правил: ключи, статусы, retry, DLQ, replay, контракты, ПДн, rollback.</p>
 <a class="btn" href="{url_for('/invariants')}">Открыть справочник инвариантов</a>
</section>
...
<section class="card" id="basics"><h2>1. Опишите бизнес-процесс</h2>
 ...
 <p class="hint">Формат поля: имя:тип|required|unique|indexed|sensitive. Пример: requestId:string|required|unique.</p><p><button type="button" class="btn ghost" data-action="suggest-basics">подставить базовые статусы, поля и ключи</button></p>
</section>
...
<section class="card" id="systems-block"><h2>2. Укажите системы-участники</h2>
 ...
 <p><button type="button" class="btn ghost" data-action="add-system">+ добавить систему</button></p>
...
<section class="card" id="process-designer"><h2>3. Соберите цепочку шагов</h2>
 ...
 <div class="templatebar" aria-label="Быстро добавить типовой шаг"><button type="button" class="btn ghost" data-action="template" data-template="rest">+ REST-вызов</button><button type="button" class="btn ghost" data-action="template" data-template="kafka">+ Kafka-событие</button><button type="button" class="btn ghost" data-action="template" data-template="db">+ запись в БД</button><button type="button" class="btn ghost" data-action="template" data-template="webhook">+ webhook/callback</button><button type="button" class="btn ghost" data-action="template" data-template="batch">+ batch/file</button><button type="button" class="btn ghost" data-action="safe-all">безопасные настройки всем шагам</button></div>
 ...
 <p><button type="button" class="btn ghost" data-action="add-step">+ пустой шаг</button>
    <button type="button" class="btn ghost" data-action="demo">заполнить примером</button></p></div>
...
<div id="errors"></div>
<p><button type="button" class="btn" data-action="submit">Проверить архитектуру и сформировать разбор</button></p>
...
def invariant_reference_page():
 ...
 <div class="refexample light"><b>Пример ошибки</b><br>{escape(example)}{extra_examples}</div>
    ...
    body = titleblock('СПРАВОЧНИК ИНВАРИАНТОВ · ЧТО НЕЛЬЗЯ ЗАБЫТЬ', active='invariants') + f'''
<section class="hero">
