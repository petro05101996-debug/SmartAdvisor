...
APP_VERSION = '8.4.4-wizard-ui-coverage-fixed'
...
<section class="card" id="chain-builder"><div class="section-head"><div><h2>3. Проверьте готовую цепочку</h2><p class="hint">После автосборки здесь появляется процесс. В простом режиме пользователь видит смысловую цепочку; техническая палитра и поля скрыты, но остаются доступны в экспертном режиме.</p></div><div class="builder-actions advanced-only"><button type="button" class="btn ghost" data-action="safe-all">безопасные настройки всем шагам</button></div></div>
