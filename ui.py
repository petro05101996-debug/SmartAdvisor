...
APP_VERSION = '8.3.1-visual-polish'
...
function renderReadiness(){
  ...
  if(hasDraftFallback) msg='Можно запустить черновой разбор: интерфейс добавит временный шаг и явно отметит допущения. Для точности выберите шаблон или добавьте цепочку.';
...
function selectScenario(kind,button){applyScenario(kind); document.querySelectorAll('.scenario').forEach(b=>{const active=b===button;b.classList.toggle('active',active);b.setAttribute('aria-pressed',active?'true':'false')}); const label=button?.querySelector('b')?.textContent||'Сценарий'; const selected=document.getElementById('selectedScenario'); if(selected)selected.textContent=kind==='blank'?'Пустой процесс. Можно заполнить минимум и запустить черновой разбор.':'Выбран шаблон: '+label+'. Проверьте название, сущность, ключ и цепочку — остальное уже заполнено безопасными дефолтами.'; showToast(kind==='blank'?'Форма очищена. Черновой разбор всё равно доступен.':'Шаблон подставлен. Теперь не заполняйте с нуля, а только поправьте под свой процесс.'); if(kind!=='blank')document.getElementById('basics')?.scrollIntoView({behavior:'smooth',block:'start'});}
    ...
    <header class="titleblock">
      <div>
        ...
        <nav class="topnav" aria-label="Основная навигация">
          ...
          <a class="{pat_cls}" href="{url_for('/patterns')}">Шаблоны проектирования</a>
        </nav>
      </div>
...
<section class="hero">
 <h2>Начните не с пустой формы, а с похожего процесса.</h2>
 <p>Интерфейс теперь работает template-first: выберите готовую ситуацию, получите уже заполненную цепочку, замените названия систем на свои и запускайте разбор. Экспертные поля не удалены — они спрятаны за расширенные настройки и продолжают попадать в тот же JSON для rule-engine.</p>
 <div class="steps3"><div class="stepbox"><b>1. Выбрать шаблон</b><br>Берём эталонный процесс вместо пустого листа.</div><div class="stepbox"><b>2. Проверить и поправить</b><br>Меняем системы, сущность, ключи и риски.</div><div class="stepbox"><b>3. Запустить разбор</b><br>Даже неполный ввод даст черновой анализ с допущениями.</div></div>
 <div class="quickstart">
  <div class="quickstart-card">
   <h3>Пример входа до заполнения</h3>
   <div class="example-flow"><div class="ex"><b>Ситуация</b>Банк отправляет документы в УК и хочет видеть, чем закончилась обработка.</div><div class="ex"><b>Что правит пользователь</b>Название систем, основную сущность, ключ поиска и статусы.</div><div class="ex"><b>Что подставляет интерфейс</b>Kafka/REST-границы, Outbox/Inbox, DLQ/replay, timeout, idempotency, correlationId.</div></div>
  </div>
  <div class="quickstart-card">
   ...
   <button type="button" class="btn ghost" data-action="demo">показать пример</button>
  </div>
 </div>
 <div class="navlinks"><a href="{url_for('/invariants')}">Открыть справочник инвариантов</a><a href="{url_for('/patterns')}">Открыть шаблоны проектирования</a><a href="{url_for('/')}">Конструктор процесса</a></div>
</section>

<nav class="guidebar" aria-label="Навигация по проектированию"><span class="guide-title">Проектирование:</span><a href="#scenario">1. шаблон</a><a href="#basics">2. смысл</a><a href="#complexity-modules">3. усложнения</a><a href="#chain-builder">4. цепочка</a><a href="#readiness">5. разбор</a></nav>
...
<section class="card" id="scenario"><h2>1. Выберите ближайшую ситуацию</h2>
 <p class="human-tip">Не ищите идеальный вариант. Выберите похожий шаблон — он уже заполнит цепочку, статусы, ключи и безопасные технические настройки. Потом замените названия систем и удалите лишние шаги.</p>
 <div class="scenario-groups">
  <div class="scenario-group"><h3>Частые бизнес-процессы</h3><p>Подходят, если надо быстро описать реальный процесс без знания терминов.</p><div class="scenarios">
  ...
  <div class="scenario-group"><h3>Ультрасложные заготовки</h3><p>Для кейсов, где обычного линейного шаблона мало: несколько потоков, join, регуляторные изменения и совместимость.</p><div class="scenarios">
 ...
 <p id="selectedScenario" class="selected-scenario">Сценарий ещё не выбран. Можно выбрать шаблон или сразу запустить черновой разбор с минимальными вводными.</p>
</section>

<section class="card" id="basics"><h2>2. Проверьте смысл процесса</h2>
...
<section class="card" id="complexity-modules"><h2>3. Что усложняет процесс?</h2>
 <p class="human-tip">Это главный упрощатель для тяжёлых кейсов: не добавляйте системы и recovery руками. Отметьте признаки процесса — UI сам достроит шаги, ключи, статусы, ограничения и безопасные дефолты.</p>
 ...
 <div class="module-panel"><div class="module-grid">
  <button type="button" class="module-btn" data-action="module" data-module="dwh"><b>+ DWH/аналитика</b><small>CDC, watermark, backfill, reconciliation report.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="legacy"><b>+ Legacy-потребитель</b><small>Dual-run, backward compatibility, cutover/rollback.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="manual_recon"><b>+ Ручная сверка</b><small>Расхождения, ожидание парной ветки, runbook, replay.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="enrichment"><b>+ REST-обогащение</b><small>Внешний справочник, cache/fallback, enriched-event.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="fanin"><b>+ Fan-in / join</b><small>Параллельные проверки, depends_on 2,3, компенсации.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="retry_dlq"><b>+ Retry/DLQ/replay</b><small>Ограниченный retry, poison messages, replay по ключу.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="audit"><b>+ Деньги/регуляторика/audit</b><small>Append-only audit journal, traceability, retention.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="outbox_inbox"><b>+ Outbox/Inbox</b><small>Transactional outbox, входящая дедупликация, replay-safe.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="contract"><b>+ Миграция контракта</b><small>v1/v2, contract tests, canary, rollback.</small></button>
  <button type="button" class="module-btn" data-action="module" data-module="security"><b>+ ПДн/security</b><small>Классификация, маскирование, RBAC, журнал доступа.</small></button>
 </div><div id="moduleStatus" class="module-status"></div></div>
</section>

<section class="card" id="chain-builder"><div class="section-head"><div><h2>4. Проверьте готовую цепочку</h2><p class="hint">После выбора шаблона здесь уже есть процесс. Переименуйте системы, удалите лишнее или добавьте шаг. В простом режиме показываем только смысл шага; технические детали спрятаны.</p></div><div class="builder-actions"><button type="button" class="btn ghost" data-action="safe-all">безопасные настройки всем шагам</button></div></div>
...
<section class="hero">
 ...
 <div class="navlinks"><a class="btn ghost" href="{url_for('/')}">Вернуться в конструктор процесса</a><a class="btn ghost" href="{url_for('/patterns')}">Шаблоны проектирования</a></div>
</section>
<section class="card">
...
<section class="hero">
 ...
 <div class="navlinks"><a class="btn ghost" href="{url_for('/')}">Вернуться в конструктор процесса</a><a class="btn ghost" href="{url_for('/invariants')}">Справочник инвариантов</a></div>
</section>
<section class="card">
 <h2>Как пользоваться</h2>
