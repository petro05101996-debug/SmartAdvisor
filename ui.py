...
APP_VERSION = '8.6.8-human-report-polish'
...
const state={mode:'quick',stage:'participants',systems:[],steps:[],stackReady:false};
state.modules=[];
let dragStepId=null;
...
function hasModule(kind){return state.modules.some(m=>m.kind===kind)}
...
function renderBranchQuestions(){
  ...
  if(!state.steps.length){box.innerHTML='<div class="branch-empty">Сначала добавьте хотя бы одну связь между участниками. После этого здесь появятся только подходящие деревья вопросов: отдельно для синхронных связей, асинхронных связей, хранения, кэша, поиска, файлов, аналитики и других веток.</div>';return;}
  const groups=STACK_BRANCH_QUESTIONS.filter(g=>branchActive(g.id));
  if(!groups.length){box.innerHTML='<div class="branch-empty">Для текущих связей дополнительных уточнений не требуется. Можно перейти к формированию стека.</div>';return;}
  box.innerHTML=groups.map(g=>'<details class="branch-question-group" open><summary><h4>'+esc(g.title)+'</h4></summary><p>'+esc(g.help)+'</p><div class="branch-question-grid">'+g.items.map(([module,label,hint])=>'<button type="button" class="branch-question-btn '+(hasModule(module)?'active':'')+'" data-action="module" data-module="'+esc(module)+'"><b>'+esc(label)+'</b><small>'+esc(hint)+'</small></button>').join('')+'</div></details>').join('');
}
function renderModules(){document.querySelectorAll('[data-action="module"]').forEach(btn=>{btn.classList.toggle('active',state.modules.some(m=>m.kind===btn.dataset.module));}); const boxes=[document.getElementById('moduleStatus'),document.getElementById('moduleStatusFlow')].filter(Boolean); const empty='Уточнения ещё не выбраны. На этом этапе показываются только те группы вопросов, которые подходят к уже построенным связям: синхронные вызовы, асинхронные события/задачи, хранение, кэш, поиск, файлы, аналитика, безопасность и эксплуатация.'; const filled='Добавлены уточнения: <b>'+state.modules.map(m=>esc(m.label)).join(', ')+'</b>. Эти ответы сужают выбор стека и будут отражены в отчёте: что выбрано, почему выбрано и почему не другой вариант.'; boxes.forEach(box=>{box.innerHTML=state.modules.length?filled:empty;});}
...
function renderInteractionBuilder(){
  ...
  if(rel){
    if(!state.steps.length){rel.innerHTML='<div class="builder-empty">Связей пока нет. Добавьте первое взаимодействие: кто отдаёт результат, кто получает и что происходит.</div>';}
    else rel.innerHTML=state.steps.map(s=>`<div class="relation-card"><b>${s.order}. ${esc(s.source_system||'Источник')} → ${esc(s.target_system||'Получатель')}</b><small>${esc(s.name||'Действие')} · ${s.blocking==='no'?'результат может появиться позже':'результат нужен сразу или в рамках шага'} · основа следующего шага: ${esc(s.dependency_basis||'результат предыдущего взаимодействия')}</small></div>`).join('');
  }
}
function inferInteractionChannel(action,timing,result){
  if(result==='save'||action==='save'||action==='update_status')return 'db';
  if(timing==='later')return 'callback';
  if(action==='notify_many')return 'kafka';
  if(action==='file')return 'file';
  if(action==='compare')return 'batch';
  return 'rest';
}
function addInteractionFromBuilder(){
  ...
  const actionLabels={request_data:'запрашивает данные у',send_data:'передаёт данные в',save:'сохраняет результат в',update_status:'обновляет статус в',wait_status:'получает поздний статус от',compare:'сверяет данные с',notify_many:'сообщает другим системам через',file:'передаёт файл в'};
  const name=(source+' '+(actionLabels[action]||'взаимодействует с')+' '+target).replace(/\s+/g,' ').trim();
  const ch=inferInteractionChannel(action,timing,result);
  const d=safeDefaultsFor(ch);
  addStep({name,source_system:source,system:source,target_system:target,channel:ch,blocking:timing==='later'?'no':d.blocking,timeout_ms:d.timeout_ms,retry:d.retry,idempotency:d.idempotency,writes_entity:(result==='save'||action==='save'||action==='update_status')?'yes':'no',depends_on:state.steps.length?String(state.steps.length):'',compensation:d.compensation,failure_policy:d.failure_policy,dependency_basis:basis},null,false);
  state.stackReady=false;
...
function addHumanStep(kind){
  ...
  addStep(step,null,false); state.stackReady=false; normalizeChainAfterStructureChange({reason:'human-step',autofillRoutes:true}); renderAll(); showToast('Шаг добавлен. Связь с предыдущим шагом создана автоматически.');
}
...
function interactionRowsHtml(options={}){
  ...
  const showStack=!!state.stackReady;
  if(!state.steps.length){
    return '<div class="schema-empty">Схема появится после добавления первой связи между участниками.</div>';
  }
  const participantNames=[...new Set(state.systems.map(x=>x.name).filter(Boolean))];
  const participants=participantNames.length?'<div class="participant-strip">'+participantNames.map(n=>`<span class="participant-pill"><b>${esc(n)}</b></span>`).join('')+'</div>':'';
  const rows=state.steps.map((s,i)=>{
    const source=safeParticipantName(s.source_system,'Источник результата');
    const executor=safeParticipantName(s.system,'Исполнитель действия');
    const target=safeParticipantName(s.target_system,'Получатель результата');
    const issue=stepIssue(s);
    const depends=dependsLabelForStep(s);
    const timing=timelineText(s);
    const stack=showStack?`<span class="schema-tag good">Стек: ${esc(labelOf(CH,s.channel))}</span>`:'<span class="schema-tag">Стек ещё не определён</span>';
    const issueTag=issue?`<span class="schema-tag warn">Проверить: ${esc(issue)}</span>`:'<span class="schema-tag good">Связь выглядит корректно</span>';
    const why=showStack?`<span class="schema-tag">${esc(humanText(s.stack_reason||stackReasonFor(s.channel,s)).slice(0,140))}</span>`:'';
    return `<div class="schema-row" data-step-id="${esc(s.id)}"><div class="schema-row-head"><span class="schema-step-no">${i+1}</span><b>${esc(s.name||'Действие процесса')}</b></div><div class="schema-route"><div class="schema-person"><small>Кто отдаёт / источник</small><b>${esc(source)}</b></div><div class="schema-arrow">→</div><div class="schema-action"><small>Кто выполняет и что делает</small><b>${esc(executor)}</b><small>${esc(simpleStepLinkText(s))}</small></div><div class="schema-arrow">→</div><div class="schema-person"><small>Кто получает результат</small><b>${esc(target)}</b></div></div><div class="schema-meta"><span class="schema-tag">${esc(timing)}</span><span class="schema-tag">${esc(depends)}</span>${stack}${issueTag}${why}</div></div>`;
  }).join('');
  return participants+'<div class="schema-flow">'+rows+'</div>';
}
...
<section class="hero">
 ...
 <div class="navlinks"><a href="{url_for('/invariants')}">Открыть справочник инвариантов</a><a href="{url_for('/patterns')}">Открыть база знаний по архитектурным решениям</a></div>
</section>
