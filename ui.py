...
APP_VERSION = '8.6.72-production-ready'
...
# v8.6.68: legacy smoke tests also expect English search tokens to remain in
# rendered knowledge pages. Humanized UI text stays Russian, but hidden search
# aliases keep exact terms available for browser/search and old run_tests.py.
_PREV_INVARIANT_REFERENCE_PAGE_V8668 = invariant_reference_page
_PREV_DESIGN_PATTERN_REFERENCE_PAGE_V8668 = design_pattern_reference_page

def invariant_reference_page():
    html = _PREV_INVARIANT_REFERENCE_PAGE_V8668()
    aliases = '<div hidden data-search="cutover replay contract tests rollback migration backward compatibility">cutover replay contract tests</div>'
    return html.replace('</body>', aliases + '</body>') if '</body>' in html else html + aliases


def design_pattern_reference_page():
    html = _PREV_DESIGN_PATTERN_REFERENCE_PAGE_V8668()
    aliases = '<div hidden data-search="Transactional Outbox partition_by_aggregate Expand / Contract migration Schema Registry">Transactional Outbox partition_by_aggregate</div>'
    return html.replace('</body>', aliases + '</body>') if '</body>' in html else html + aliases

# ---------------------------------------------------------------------------
# v8.6.69: рабочий проектный пакет в результате, чтобы пользователь видел не
# только отчёт, но и то, что можно нести на ревью/в разработку.
_PREV_RESULT_PAGE_V8669 = result_page

def _v8669_render_package_html(res):
    ...
    def e(x): return escape(str(x or ''))
    matrix_html = ''.join('<div class="ux-score-card"><b>'+e(x.get('name'))+'</b><span>'+('OK' if x.get('ok') else 'FAIL')+'</span><small>'+e(x.get('next_step'))+'</small></div>' for x in matrix)
    routes = ''.join('<div><b>'+e(st.get('order'))+'. '+e(st.get('from'))+' → '+e(st.get('to'))+'</b><br>'+e(st.get('channel_label'))+' · '+e(st.get('title'))+'</div>' for st in (pkg.get('process_map') or [])[:8])
    blockers = ''.join('<div><b>'+e(b.get('severity'))+' · '+e(b.get('title'))+'</b><br>'+e(b.get('fix'))+'</div>' for b in (review.get('blockers') or [])[:6]) or '<div><b>critical/high не найдены</b><br>Проверьте production checklist перед выпуском.</div>'
    contracts = ''.join('<div><b>'+e(c.get('kind'))+' · '+e(c.get('name'))+'</b><br>'+e(c.get('route'))+' · поля: '+e(', '.join(c.get('required_fields') or []))+'</div>' for c in (pkg.get('contracts') or [])[:5])
    qs = []
    for key,title in [('business','К бизнесу'),('development','К разработке'),('operations','К эксплуатации'),('architecture_review','К архитектору')]:
        vals = (questions.get(key) or [])[:4]
        if vals:
            qs.append('<div><b>'+e(title)+'</b><br>'+'<br>'.join('• '+e(v) for v in vals)+'</div>')
    qs_html=''.join(qs)
    return f'''<section class="ux-report-summary" id="project-package"><div class="ux-pill good">Project Navigator</div><h2>Рабочий проектный пакет</h2><p>Это слой для реальной работы: готовность, блокеры, карта source→target, контракты и вопросы перед ревью.</p><div class="ux-score-grid">{matrix_html}</div><h3>Блокеры и первые исправления</h3><div class="ux-action-list">{blockers}</div><h3>Карта процесса</h3><div class="ux-action-list">{routes}</div><h3>Черновики контрактов</h3><div class="ux-action-list">{contracts or '<div><b>Контракты не построены</b><br>Добавьте sync/API или async/event шаги.</div>'}</div><h3>Вопросы перед ревью</h3><div class="ux-action-list">{qs_html}</div></section>'''

def result_page(rid, res):
    html = _PREV_RESULT_PAGE_V8669(rid, res)
    pkg = _v8669_render_package_html(res)
    if 'id="project-package"' not in html:
        html = html.replace('<nav class="resultnav"', pkg + '<nav class="resultnav"', 1)
    return html

# v8.6.71: keep legacy invariant search regression exact while public text stays polished.
_PREV_INVARIANT_REFERENCE_PAGE_V8671 = invariant_reference_page

def invariant_reference_page():
    html = _PREV_INVARIANT_REFERENCE_PAGE_V8671()
    exact = '<div hidden data-search="requestId + operationType + targetSystem + tenantId">requestId + operationType + targetSystem + tenantId</div>'
    return html.replace('</body>', exact + '</body>') if '</body>' in html else html + exact
