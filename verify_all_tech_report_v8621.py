...
must(('Сохранить запись формата «ключ-значение» в DynamoDB». Основной способ взаимодействия: Хранилище ключ-значение' in md) or ('Сохранить ключ-значение в DynamoDB». Основной способ взаимодействия: Хранилище ключ-значение' in md), 'DynamoDB must not be classified as Vault/KMS')
...
# 2. Сквозные контроли отделены от бизнес-цепочки.
...
for expected in [
    ...
    'Реплика базы данных для чтения', 'Redis как кэш', 'Redis как распределённая блокировка', 'Redis как короткая очередь задач',
