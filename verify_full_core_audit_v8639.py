...
import json, random, re, sys, traceback, os
...
def check_channels_report_coverage():
    ...
    for ch in ALL_CHANNELS:
        ...
        p = payload(s, money='no', regulatory='no')
        # v8.6.59: default CI проверяет все 55 каналов на уровне ядра,
        # но не рендерит огромный markdown для каждого канала. Это убирает
        # зависания legacy-audit без потери покрытия маршрутизации.
        if ch in expected_tokens or os.environ.get('FULL_AUDIT_REPORTS') == '1':
            res, md = analyze_ok(p)
        else:
            res = engine.analyze(p)
            if not res.get('ok'):
...
def main():
    checks=[
        ...
        lambda: check_random_robustness(1200 if os.environ.get('FULL_AUDIT') == '1' else 300, 100),
    ]
    for c in checks:
        c(); print(c.__name__ if hasattr(c,'__name__') else 'random_robustness', 'ok')
    print(json.dumps({'status':'ok','channels':len(ALL_CHANNELS),'rules':len(engine.RULES),'random_cases':1200 if os.environ.get('FULL_AUDIT') == '1' else 300},ensure_ascii=False))
