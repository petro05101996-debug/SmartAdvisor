...
for fragment in ('Тренажёр системного аналитика','Каталог учебных кейсов MVP','learning-grid'):
    if fragment not in home:
        issues.append('home_missing_'+fragment)
summary=[]
for case in cases:
    page=learning_case_page(case['id'])
    for fragment in (case['title'], 'Проверить моё решение', 'Markdown-отчёт тренажёра', 'REFERENCE_PAYLOAD'):
    ...
    for fragment in ('Учебный разбор','Профиль навыков','Что мешает production-ready','Полный архитектурный отчёт ядра'):
...
print(json.dumps({'ok': not issues, 'case_count': len(cases), 'summary': summary, 'issues': issues}, ensure_ascii=False, indent=2))
if issues:
    raise SystemExit(1)
