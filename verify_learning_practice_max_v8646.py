...
def ui_static_check():
    ...
    if 'caseSearch' not in home or 'levelFilter' not in home or 'trackFilter' not in home or 'filterTrainerCases' not in home:
    ...
    for frag in ('Проверить выбранное решение','Вопросы интервьюера','Подсказка','Показать эталон'):
...
def browser_filter_check():
    # v8.6.62: this older verifier is part of the fast release gate, so it keeps
    # browser automation optional. Dedicated UI/browser smoke is covered by
    # verify_trainer_max_ux_v8658.py and verify_ui_full_v8643.py.
    if os.environ.get('FULL_BROWSER') != '1':
        return {'ok': True, 'browser': 'skipped: FULL_BROWSER=1 enables legacy browser filter check'}
    try:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for name, vp in [('desktop',{'width':1366,'height':900}),('tablet',{'width':768,'height':1024}),('mobile',{'width':390,'height':844})]:
                ...
                expect(page.locator('.trainer-hero.maxux')).to_be_visible()
                expect(page.locator('text=Начать первый кейс')).to_be_visible()
                page.locator('#catalog').evaluate('el => el.open = true')
                expect(page.locator('#caseSearch')).to_be_visible()
                vals=page.evaluate("""() => ({
                    sw: document.documentElement.scrollWidth,
                    cw: document.documentElement.clientWidth,
                    cards: document.querySelectorAll('.trainer-case-card').length,
                    visible: [...document.querySelectorAll('.trainer-case-card')].filter(x => getComputedStyle(x).display !== 'none').length
                })""")
                ...
                filtered=page.evaluate("""() => [...document.querySelectorAll('.trainer-case-card')].filter(x => getComputedStyle(x).display !== 'none').length""")
                assert 0 < filtered < vals['cards'], (name, filtered, vals['cards'])
                page.fill('#caseSearch', '')
                page.select_option('#levelFilter', 'Senior')
                page.evaluate('filterTrainerCases()')
                senior=page.evaluate("""() => [...document.querySelectorAll('.trainer-case-card')].filter(x => getComputedStyle(x).display !== 'none').length""")
...
def live_api_check():
    ...
    try:
        ...
        assert 'caseSearch' in home and 'Весь каталог кейсов:' in home and 'Начать первый кейс' in home
        ...
        assert 'REFERENCE_PAYLOAD' in page and 'Проверить выбранное решение' in page and 'Показать эталон' in page
