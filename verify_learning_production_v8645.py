...
def direct_checks():
    ...
    sample=[]
    for c in cases[:8 if os.environ.get('FULL_AUDIT') == '1' else 5]:
...
def live_checks():
    ...
    try:
        ...
        assert 'Тренажёр системного аналитика' in home and 'Как пользоваться' in home and 'Весь каталог кейсов' in home
        ...
        assert 'Проверить выбранное решение' in page and 'Показать эталон' in page
...
def ui_static_checks():
    ...
    required=['Тренажёр системного аналитика','Проверить выбранное решение','Сформировать отчёт','Открыть базу знаний']
    blob='\n'.join(pages)
