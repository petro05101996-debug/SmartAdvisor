...
import atexit, json, os, subprocess, sys, time, urllib.request, shutil, tempfile
...
APP_DIR_TMP=tempfile.mkdtemp(prefix='smartadvisor_v8653_')
...
def browser_checks():
    ...
    with sync_playwright() as p:
        ...
        for name, vp in [('desktop',{'width':1366,'height':900}),('tablet',{'width':768,'height':1024}),('mobile',{'width':390,'height':844})]:
            ...
            page.evaluate("document.getElementById('catalog').open=true")
            expect(page.locator('#caseSearch')).to_be_visible()
            page.fill('#caseSearch', 'Kafka')
            page.evaluate('filterTrainerCases()')
            vals=page.evaluate("() => ({sw:document.documentElement.scrollWidth,cw:document.documentElement.clientWidth, cards:[...document.querySelectorAll('#learningGrid .trainer-case-card:not(.hidden-by-saas)')].length})")
            assert vals['sw'] <= vals['cw'] + 2, (name,'home_overflow',vals)
            assert vals['cards'] > 0, (name,'filter_no_cards')
            if keep_screenshots and name=='desktop':
            ...
            expect(page.get_by_role('button', name='Вопросы интервьюера')).to_be_visible()
            ...
            page.locator('.visual-control').first.check()
            checked=page.evaluate("() => document.querySelectorAll('.visual-control:checked').length")
            total=page.evaluate("() => document.querySelectorAll('.visual-control').length")
            assert checked == 1 and total >= 3, (checked, total)
            if keep_screenshots and name=='desktop':
