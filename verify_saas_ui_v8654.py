# -*- coding: utf-8 -*-
"""Пользовательская проверка упрощённого UX тренажёра v8.6.57.
...
import atexit, json, os, subprocess, sys, time, urllib.request, shutil, tempfile, re, multiprocessing
...
APP_DIR_TMP = tempfile.mkdtemp(prefix='smartadvisor_v8654_')
atexit.register(lambda: shutil.rmtree(APP_DIR_TMP, ignore_errors=True))
APP_ENV = {**os.environ, 'PORT': str(PORT), 'HOST': '127.0.0.1', 'APP_DIR': APP_DIR_TMP}
...
def _browser_static_checks_impl():
    ...
    out = []
    screenshots = []
    keep_screenshots = os.environ.get('KEEP_UI_SCREENSHOTS') == '1'
    ...
    with sync_playwright() as p:
        ...
        for name, vp in [('desktop', {'width': 1366, 'height': 900}), ('tablet', {'width': 768, 'height': 1024}), ('mobile', {'width': 390, 'height': 844})]:
            ...
            assert not re.search(r'(^|\n)\s*ed\s*(\n|$)', body_text.split('Мой прогресс')[0]), f'{name}: broken standalone text artifact is visible'
            
            if page.locator('#caseSearch').count() and not page.locator('#caseSearch').is_visible():
                page.get_by_text('Весь каталог кейсов').click()
            page.fill('#caseSearch', 'Kafka')
            page.evaluate('filterTrainerCases()')
            home = page.evaluate("() => ({sw:document.documentElement.scrollWidth,cw:document.documentElement.clientWidth,cards:[...document.querySelectorAll('#learningGrid .trainer-case-card:not(.trainer-hidden)')].length})")
            assert home['sw'] <= home['cw'] + 2, (name, 'home_overflow', home)
            assert home['cards'] > 0, (name, 'filter_no_cards')
            if keep_screenshots and name in ('desktop', 'mobile'):
            ...
            assert 'buildVisualSolution(\'weak\')' not in page.content(), f'{name}: old weak draft autostart is still present'
...
def browser_static_checks(timeout_sec: int = 120):
    """Run legacy Playwright UI smoke only on demand.

    v8.6.62 uses verify_ui_browser_smoke_v8662.py as the stable release browser
    gate. This older verifier remains available with FULL_BROWSER=1, but does
    not block the default release check.
    """
    if os.environ.get('FULL_BROWSER') != '1':
        return {'browser': 'skipped: FULL_BROWSER=1 enables legacy SaaS browser smoke'}
    q = multiprocessing.Queue()
    def _target():
        try:
            q.put((True, _browser_static_checks_impl()))
    ...
    if proc.is_alive():
        ...
        return {'browser': 'skipped_timeout', 'reason': f'Playwright UI smoke timed out after {timeout_sec}s'}
    if q.empty():
        return {'browser': 'skipped_no_result'}
    ok, payload = q.get()
    if not ok:
