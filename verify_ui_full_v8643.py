...
def main() -> int:
    ...
    if os.environ.get('FULL_BROWSER') != '1':
        # v8.6.62: keep the legacy full UI script deterministic in release CI.
        # Browser automation for the simplified trainer is covered separately by
        # verify_trainer_max_ux_v8658.py. Set FULL_BROWSER=1 to run the original
        # deep Playwright click-through locally.
        form = ui.form_page()
        inv = ui.invariant_reference_page()
        pat = ui.design_pattern_reference_page()
        sample_payload = {
            "meta": {"name": "UI static full check", "entity": "Order", "lookup_keys": "orderId", "statuses": "NEW,DONE"},
            "systems": [{"name": "Клиент", "role": "external"}, {"name": "Сервис", "role": "internal"}],
            "steps": [{"order": 1, "name": "Клиент передаёт заявку", "source_system": "Клиент", "system": "Сервис", "target_system": "Сервис", "channel": "rest", "blocking": "yes", "retry": "auto", "idempotency": "key", "timeout_ms": "800"}],
        }
        res = analyze(sample_payload)
        result_html = ui.result_page("1234567890abcdef1234567890abcdef", res)
        checks = {
            "form_has_participants": "1. Участники процесса" in form and "Добавить инициатора" in form,
            "form_has_report_flow": "Сформировать отчёт" in form or "Дальше: отчёт" in form,
            "result_has_priority": "Что сделать в первую очередь" in result_html,
            "invariants_cards": inv.count("details") >= 5 or inv.count("refcard") >= 5,
            "patterns_cards": pat.count("pattern-card") >= 5 or pat.count("refcard") >= 5,
        }
        bad = [k for k, v in checks.items() if not v]
        if bad:
            raise AssertionError({"failed_static_checks": bad})
        output = {"ok": True, "version": ui.APP_VERSION, "live_api_id": live_id, "browser": "skipped: set FULL_BROWSER=1 for legacy deep click-through", "static_checks": checks}
